...
# === V10: 対照群を「新バフェット型20社」へ。重みは期首加重(主)・期末加重(参考)の両建て ===
...
COMP = json.load(open(ED / "control_comparison_v10.json", encoding="utf-8"))
# 主対照=新バフェット型20社・期首加重(C)、参考併記=同20社・期末加重(E)、参考=純正グレアム20(CG)
O3, O1 = COMP["ours"]["3y"], COMP["ours"]["1y"]
C3, C1 = COMP["control_buffett20_start"]["3y"], COMP["control_buffett20_start"]["1y"]  # 主対照(期首加重)
E3, E1 = COMP["control_buffett20_end"]["3y"], COMP["control_buffett20_end"]["1y"]      # 参考(期末加重)
CG3, CG1 = COMP["control_graham"]["3y"], COMP["control_graham"]["1y"]       # 純正グレアム(参考)
LV3 = COMP["ours_vol_matched"]["3y"]                                        # リスク同量化(参考)
SENS = COMP["sensitivity"]
_WV = COMP["weighting_variants"]
PH = COMP["phases"]
def _r1(x):
    """0.05刻みの丸め揺れ(13.95→13.9/14.0)を防ぐため、表示前に半数切り上げで1桁へ固定する。"""
    return round(x + 1e-9, 1)


D3 = _r1((O3["ann_return"] - C3["ann_return"]) * 100)   # 対主対照 超過(3年)
D1 = _r1((O1["ann_return"] - C1["ann_return"]) * 100)   # 対主対照 超過(1年)
DE3 = _r1((O3["ann_return"] - E3["ann_return"]) * 100)  # 対期末加重 超過(3年・マイナス)
DE1 = _r1((O1["ann_return"] - E1["ann_return"]) * 100)  # 対期末加重 超過(1年)
DG3 = _r1((O3["ann_return"] - CG3["ann_return"]) * 100)  # 対グレアム超過(3年)
DG1 = _r1((O1["ann_return"] - CG1["ann_return"]) * 100)  # 対グレアム超過(1年)
G1 = (O1["ann_return"] - O1["topix_ann_return"]) * 100
G3 = (O3["ann_return"] - O3["topix_ann_return"]) * 100
SIG = json.load(open(ED / "significance_v10.json", encoding="utf-8"))
# SIG構造: ours_vs_buffett20_start / ours_vs_buffett20_end / ours_vs_graham / ours_vs_topix
ROB = json.load(open(ED / "robustness_v7.json", encoding="utf-8"))

# ---- v11で追加した正典(分析WPの作業で導出。V11_PLAN.md §1のA〜I) ----
SAT = json.load(open(ED / "saturation_v11.json", encoding="utf-8"))       # E 飽和の機構 / F 自PF監査
PIT = json.load(open(ED / "layer1_pit_v11.json", encoding="utf-8"))       # A 時点外検証
FAC = json.load(open(ED / "factors_v11.json", encoding="utf-8"))          # C ファクター帰属
CST = json.load(open(ED / "costs_v11.json", encoding="utf-8"))            # B MDE / G コスト・DSR
PLA = json.load(open(ED / "layer2_placebo_v11.json", encoding="utf-8"))   # D ランダム化推論
SRV = json.load(open(ED / "survivorship_v11.json", encoding="utf-8"))     # H 生存者性
APX = json.load(open(ED / "appendix_v11.json", encoding="utf-8"))         # I 証拠水準×売上成長

_sat, _own = SAT["saturation"], SAT["own_portfolio_audit"]
_lad = SAT["evidence_ladder"]
...
def banner1(text, newpage=False, before=10):
    HEADINGS.append((text, 1))
    p = para(before=before, after=5, keep=True)
...
def banner3(text):
    p = para(before=4, after=2, keep=True)
...
def body(text, size=10.5, after=4):
    """本文段落。**…** で囲んだ範囲はゴシック太字にする(v11で追加)。
    v10までは太字記法が無く、記法を書くとアスタリスクがそのまま印字されていた。"""
    p = doc.add_paragraph()
    ...
    for k, seg in enumerate(text.split("**")):
        if seg:
            X.setfont(p.add_run(seg), GO if k % 2 else MIN, size, bool(k % 2), BLACK)
    return p
...
def point(text):
    ...
    for k, seg in enumerate(text.split("**")):   # v11: **…** を太字に
        if seg:
            X.setfont(p.add_run(seg), GO if k % 2 else MIN, 10, bool(k % 2), BLACK)
...
def formula(num, title, latex, defn, hitokoto=None):
    """v10: hitokoto=「ひとこと訳」(式を日常語に置き換えた1行)。全式に必須。"""
    ...
    if hitokoto:
        X.setfont(p2.add_run("ひとこと訳　"), GO, 10, True, TEALC)
        X.setfont(p2.add_run(hitokoto), MIN, 10, False, BLACK)
        p3 = c2.add_paragraph(); X._clear_indent(p3)
        p3.paragraph_format.space_before = Pt(0); p3.paragraph_format.space_after = Pt(2)
        X.setfont(p3.add_run(defn), MIN, 10, False, BLACK)
    else:
...
body("本レポートの軸は一つだけである。目指す先が「三世代の堀」(何を買うか)、そこへ至る道順が「守破離」(どう選ぶか)であり、堀が主役で守破離は選ぶ手順の名前にすぎない。守破離(しゅはり)とは日本の芸道の修行段階で、守＝型を守る・破＝型を破る・離＝独自の境地へ、を指す。守ではバフェットの品質基準(先行研究の式)だけで完成した堀を5社選び、破ではその型を破って割安×変革の会社(変わる堀)を、離では独自の物差しで新しい堀(生まれる堀)を選び、両立型3社・分散役2社を加えて最終20社を組んだ。選定後に成績を見て銘柄を入れ替える「後からの選び直し」はしていない(あとから成績の良い銘柄だけを拾えば、どんな選び方でも立派に見えるからだ)。その代わり、この選び方が条件を一つ変えたくらいでは崩れないことを、三通りの頑健性検査(品質ゲートの判定の目安を変えること・配分の重み方式・相場局面)で確かめた(詳細は第Ⅱ章末)。")
body(f"結論を先に言う。私たちは「超える」を四つの条件で定義した。①バフェットと同じ土俵に立つ、②彼の式では選ばれない会社を同じ水準の数字の裏づけで選ぶ、③選定が壊れにくい――この三つの設計条件は本文で確かめた。残る④成績は、バフェットの品質基準だけで選んだ20社『新バフェット型』を自作し、先に決めた物差しで比較した。過去データの自己検証では、当時実際に組めた新バフェット型(3年前の会社の大きさで配分した20社)を、リターン・効率・下落の小ささのすべてで上回った(3年で年＋{D3:.0f}ポイント。直近1年の差はｔ＝{SIG['1y']['ours_vs_buffett20_start']['t_newey_west']:.2f}で統計的にも確実)。グレアム型と市場平均(ＴＯＰＩＸ)にも有意に上回った。ただし正直に書く――同じ20社に「上がった後の大きさ」で配分し直した仮想の対照には、3年のリターンで{-DE3:.0f}ポイント届かない(効率と下落の小ささでは上回る)。これはすべて過去の自己検証であり、将来の市場超過の証明ではない――この限界は隠さず記す。")
...
body("きっかけは、一つの会社だった。santec Holdings(6777)――光通信の部品をつくる会社である。地味な部品メーカーと見られてきたが、いま、生成ＡＩのデータセンター内をつなぐ高速の光配線の担い手として需要が急拡大している。データが詰まれば計算全体が止まる――その急所を、精密な光の部品技術が握った。完成したはずの部品事業の内側で、ＡＩという新しい堀が生まれていた。")
body("同じ頃、東京証券取引所は上場企業に「資本コストや株価を意識した経営」を求め、長く割安に放置された企業が変わり始めた。堀は止まった絵ではない。変わる堀があり、生まれる堀がある。ならば、いまの堀だけを測るバフェットの物差しを、時間の方向へ延長できないか。これが私たちの問いである。")

banner2("2．バフェットの規律 ― 何がすごいのかを分解する")
body("「超える」と言う前に、まず相手を正しく知る。バフェットの投資は、大きく四つの部品でできている。")
bullet("堀(持続する競争優位)", "まねされにくい強み。ブランドの力(例:コカ・コーラ)、乗り換えの手間、利用者が増えるほど価値が増す仕組み、コストの安さ、特許や免許。バフェット(2007)は「永続的な堀」を持つ企業だけを買うと明言する。")
bullet("安全域(高すぎない価格)", "どんな良い会社も、高く買いすぎれば損をする。会社の価値より安い価格で買い、見立て違いに備える。冒頭の1989年の言葉は、価格より会社の質を優先するという順序も示している。")
bullet("能力の輪(分かるものだけ買う)", "自分が理解できる事業だけを対象にし、理解できない熱狂には乗らない。")
bullet("長期保有(複利で増やす)", "「私たちの好みの保有期間は永遠である」(バフェット、1988)。頻繁に売買せず、堀が守る利益の再投資で増やす。")
body("バフェットの物差しには死角もある。彼が測るのは割安さ・優良さ・値動きの穏やかさ――どれも「いま完成している堀」であり、これから変わる堀・生まれる堀は映らない。私たちが加える視点は三つ――①時間軸(堀の一生を見る)、②証拠主義(言葉ではなく数字の裏づけで測る)、③分散(一つの未来に賭けない)。これを彼と同じ規律の厳しさで実行することが本レポートの挑戦である。")

banner2("3．背景 ― 私たちの追い風となる二つの構造変化")
body("第一の変化は資本効率改革である。東京証券取引所(2023)の要請は、伊藤レポート(経済産業省、2014)以来の資本収益性重視の流れを決定づけ、ＰＢＲ(株価純資産倍率。株価が会社の純資産の何倍かを示す)1倍割れ企業の経営改革を促した。割安に放置された企業の中から、本当に変われる企業を見分けられれば、それは「変わる堀」への投資になる。")
body(f"第二の変化はＡＩを起点とする産業構造の転換である。生成ＡＩは、半導体・光通信・データセンター・電力・品質保証といった基盤産業に、実際の需要という追い風を与えている。ただし「ＡＩ」と口にする企業がすべて堀を持つわけではない。私たちは既製のＡＩ関連スコアを全上場{_sat['n_firms']:,}社で検算し、**この物差しが企業を並べられない**ことを確かめた――点数は{_sat['n_distinct_values']}通りの値しか取らず、{_sat['sector_determinism']['share_exactly_at_sector_median']*100:.1f}%の企業が自業種の中央値と完全に一致する。原因は三つ重なる。①照合先が**開示文書ではなく社名・業種名などの属性**であること。②特定業種というだけで加点される仕組みがあること(業種名は照合対象でもあり二重に効く)。③点数の四分の一を占める唯一の企業ごとの連続量(研究開発費比率)が{_sat['mechanism']['rd_ratio_non_missing']}社しか値を持たず残りはゼロ埋めであること。業種の中で差がつく入力が残らない――**企業の実力を測るはずの点数が、算術的には業種のラベルだった**。接点と堀を見分ける仕組み――開示まで遡って実需を確かめる証拠主義――が要る。")

banner2("4．三世代の堀 ― 私たちが求める企業像")
body("以上から、私たちは投資対象を三つの世代で定義する――すでに完成した堀(バフェットの規律で測れる企業)、資本効率改革で変わる堀(割安×改善の証拠を持つ企業)、構造変化から生まれる堀(ＡＩ基盤への実需接続を証拠で示せる企業)。この三世代を一つに束ね、どれか一つの未来に賭けない。これが「三世代の堀」である。")
fig("fig1_moat_v10.png", "図表Ⅰ-1　三世代の堀 ― 完成・変化・新生を一つに束ねる",
    "注：上段は三世代の名前、下段は中身。両立型・分散役を加えた合計20社で構成する。この三世代(何を買うか)を、第Ⅱ章では守・破・離の順番(どう選ぶか)で選び出す。筆者作成。", maxw=132)
...
X.setfont(p_h.add_run("完成した堀・変わる堀・生まれる堀を証拠で選んで一つに束ねたポートフォリオは、堀の“現在”だけを測る新バフェット型の選定を、リスク調整後の効率(シャープ)と下落の小ささで上回る。過度な集中を避けた三世代の分散が攻めと守りの両面で効く――これが三世代を等しく束ねる理由である。"), MIN, 10.5, True, BLACK)

banner2("5．何をもって「バフェットを超える」とするか ― 四つの条件")
body("「超える」を、確かめられる四つの条件で先に定義しておく。本レポートの残りはこの定義の検証である。")
table(["条件", "内容", "どこで確かめるか"],
      [["① 同じ土俵に立つ", "先行研究の式でバフェット流の5社を選び、その5社を実際に20社の中に組み入れる", "第Ⅱ章「守」"],
       ["② 死角を補う", "バフェットの式では選ばれない会社にも、守の5社と同じ水準の数字の裏づけを要求して選ぶ", "第Ⅱ章「離」"],
       ["③ 壊れにくい", "一つの仕掛け・一つのテーマに依存しない。条件を一つ変えても20社中11社以上が維持される", "第Ⅱ章末の検査"],
       ["④ 総体で上回る", "主対照『新バフェット型』(彼の品質基準だけで選んだ20社。配分は3年前の会社の大きさ)に、リターン・効率・下落の小ささで上回る。グレアム型・市場には有意に上回る。物差しは比較前に宣言", "第Ⅲ章の検証"]],
      [34, 102, 34], fs=10)
para("図表Ⅰ-2　「超える」の定義 ― 四つの条件と検証の場所。", MIN, 10, after=5)

banner2("6．検証の順番 ― 守・破・離")
body("守=条件①、離=条件②、章末の検査=条件③、第Ⅲ章の対照比較=条件④に対応する(下表)。")
table(["段階", "選ぶ堀とやること", "どこまでが借り物か", "検証する条件"],
      [["守", "完成した堀5社を先行研究の式だけで選ぶ", "式も使い方も先行研究のまま", "条件①"],
       ["破", "割安×変革の会社(変わる堀)5社を選ぶ", "式は借り物、使い方は私たちの判断", "(条件②の一部)"],
       ["離", "生まれる堀＋両立・分散の15社を独自の物差しで選ぶ", "部品は借り物、組み方が独自", "条件②"]],
      [16, 72, 56, 26], fs=10)
para("図表Ⅰ-3　守・破・離の分担 ― 独自性を最後(離)に置く。本文の数字は選定過程の記録から照合して転記した。", MIN, 10, after=5)
...
fig("fig2_overview_v9.png", f"図表Ⅱ-1　スクリーニングの全体設計図 ― {F9['n_nonfin']:,}社から20社へ",
    "注：スクリーニング0は全枝共通の関所。1〜4は同じ出発点(1,791社)から役割別に選ぶ並列の枝であり、番号は絞り込みの直列順ではなく読み順。全社数は選定と同一条件で再計算し、最終20社と一致することを確かめた。出所：選定記録より筆者作成。")
...
body(f"正直な限界も先に書く。出発点の{F9['n_nonfin']:,}社は直近時点の上場一覧で、期間中に上場廃止となった会社を含まない(生存者バイアス)。また履歴3年の関所は、上場3年未満の若い会社を機械的に外す――生まれる堀の候補を、検証可能性と引き換えに少し狭めている。", after=6)

step_chip(1, f"守 ― 完成した堀を品質の七関門で選ぶ({F9['n_base']:,}社→5社)")
body("ねらい: バフェットの規律「良い会社を、高すぎない価格で買い、長期保有に耐えない企業を避ける」を、先行研究の式だけで再現する。私たちの判断や調整は加えず、七つの関門を一つずつ通す。ここで選ぶ5社は「何を超えたのか」を測る基準線であり、以降一度も入れ替えない。")
def verdict(text):
    p = para(after=6)
...
body("七つの関門を、式そのものから一つずつ見ていく。どの式も先行研究のまま使う。")
...
formula("1", "ＲＯＥ ― 資本の収益率(Frazzini, Kabiller and Pedersen, 2018)",
        ...
        hitokoto="株主のお金100円が1年で何円の利益を生むか。15円以上を合格とする。")
...
formula("2", "営業利益率 ― 価格支配力(堀)の代理(Asness, Frazzini and Pedersen, 2019)",
        ...
        hitokoto="売上100円のうち何円が本業のもうけか。10円以上なら値下げ競争に強い。")
...
formula("3", "自己資本比率 ― 財務の安全(Asness, Frazzini and Pedersen, 2019)",
        ...
        hitokoto="会社の財産のうち、借金に頼らない部分の割合。半分以上なら不況に強い。")
...
formula("5", "営業キャッシュフロー ― 利益の質(Sloan, 1996)",
        ...
        hitokoto="帳簿の上の利益ではなく、本業で現金が実際に入っているか。")
...
formula("7", "60日平均売買代金 ― 売買のしやすさ(Amihud, 2002)",
        ...
        hitokoto="その株が1日にいくら分売買されているか＝買いたい量を買えるか。")
...
table(["守の関門", "通過社数", "落ちる主な理由"],
      ...
      [66, 20, 70], fs=10)
para(f"図表Ⅱ-2　守の結果 ― 適格{F9['n_base']:,}社が{F9['shu']['n_quality']}社、価格ランク可能{F9['shu']['n_priceable']}社まで絞られる。関門7(売買代金)は共通の関所(スクリーニング0)で先に適用済み。出所：選定記録より筆者作成。", MIN, 10, after=5)
g = DATA["6920"]
...
table(["品質関門", "レーザーテックの値", "判定の目安", "判定"],
      ...
      [42, 44, 40, 20], fs=10)
para("図表Ⅱ-3　通し数値例：レーザーテック(6920)の品質関門。出所：選定記録より筆者作成。", MIN, 10, after=6)
...
body(f"最後の絞り込み: 品質{F9['shu']['n_quality']}社のうち価格ランクが可能な{F9['shu']['n_priceable']}社を、割安×優良の複合順位(収益力ＲＯＥと益回りの順位和＝Greenblattの考え方)で並べ、同一業種は原則2社までとして上位5社を固定した。選ばれたのは、"
     ...
     + f"の5社である。なお、この順位を付けられた{F9['shu']['n_priceable']}社すべてが、第Ⅲ章で比較する主対照『新バフェット型』である――社数を私たちの都合で切らないため、上位何社という線引きをせず、関門を通った全社をそのまま対照にした。守の5社はその上位5社に一致する。比較相手の一部を自分の20社に組み入れる、という条件①はこの一致で担保される。")
...
para("図表Ⅱ-4　守Top5の中身 ― 割安×優良の複合順位(ＲＯＥの順位＋益回りの順位)の上位5社。同一業種は2社まで。出所：選定記録より筆者作成。", MIN, 10, after=5)
fig("fig2_shu_v9.png", f"図表Ⅱ-5　スクリーニング1(守)の絞り込み ― 適格{F9['n_base']:,}社から完成した堀Top5へ",
    "注：棒の長さは残った会社数(対数目盛)。1-1〜1-6は品質の六関門、1-7は価格ランク可能、1-8で5社に固定。出所：選定記録より筆者作成。")
...
formula("8", "順位化 ― 市場の中の相対的な位置に置き換える(Fama and French, 1993)",
        ...
        hitokoto="単位の違う指標を比べるため、実数を「全体の中で何番目か」に置き換える。")
verdict("よって、7つの部品すべてを0〜1の順位点に変換して比べる。式の定義は一切変えない。")
banner3("手順2　順位点をどう束ねるか ― 重みつき合成点(変革の論理で設計)")
body("各順位点に重みを付けて足し合わせ、危険な兆候(データ欠け・極端に小さい会社など)には減点を与える。重みとは、どの項目を何倍重視するかの係数である。複数のシグナルを一つの合成点に束ねる形は、9項目の合計点で勝ち組と負け組を分けたPiotroski (2000)のＦスコアと同型であり、私たちはその「合計」を「重みつき合計−減点」に一般化した(式9)。重みは過去リターンへの当てはめでは決めない――どの重みなら過去の成績が良かったか、という決め方は後出しの罠だからである。『安さ』より『改善の実行』を重くする、という変革の論理から概念的に設計し、決めた後に順位の安定性を監査した(実際の値は式10・式11の欄に示す)。", after=2)
formula("9", "合成点 ― 候補づくりのための重みつき合計(Piotroski, 2000)",
        ...
        hitokoto="複数の順位点に、重視する度合い(＝重み)を掛けて一つの点数に足し合わせる。")
verdict("よって、合成点の上位から候補リストを作る。ただし、この点数で最終選定はしない。")
banner3("手順3　順位の付け方を変えても結果は同じか ― 頑健さの監査")
body("順位化のやり方は一つではない。市場全体での順位・業種の中での順位・外れ値に強い標準化で付け直し、顔ぶれが入れ替わらないかを確かめた。付け方に敏感な会社は離の個別確認へ引き継いだ。", after=2)
banner3("式(10)の導出 ― 変わる堀の定義を、測れる四つの要素に分ける")
body("式(9)の合成点を、変わる堀に合わせて具体化したものが式(10)である(破の選定に使う独自の物差し)。ここから式(10)〜(12)は私たちの設計だが、導出は三つの原則に従う――第一に、部品は先行研究と前掲の順位点の再利用。第二に、重みは成績で決めない(過去リターンへの当てはめは後出しの罠)。第三に、点数(魅力の大きさ)と証拠(裏づけの強さ)を混ぜない。変わる堀の定義『割安に放置されているが、資本効率改革の実行でこれから評価が変わる会社』を、六つの測れる要素に分ける――(a)割安ギャップＶ(いま割安か＝純資産比・益回りの順位点)、(b)資本効率の改善Ｃ(ＲＯＥ・利益率が良くなっているか)、(c)株主還元の強化Ｒ(配当・自社株買いの実行)、(d)改革シグナルＫ(中期計画・ＰＢＲ改善策の開示)、(e)実行の確かさＥ(赤字や債務の重さがなく、改革を続けられるか)、(f)ワナ耐性Ｑ(安いだけでなく本業の質があるか)。これらを重みつきで束ね、『安いだけの会社』を弾く減点を差し引く。", after=2)
formula("10", "変わる堀の点数(破の選定に使用・私たちの設計)",
        r"S^{\mathrm{Trans}}_i = w_V V_i + w_C C_i + w_R R_i + w_K K_i + w_E E_i + w_Q Q_i - P^{\mathrm{Trap}}_i",
        "Ｖ＝割安ギャップ(重み0.20)　Ｃ＝資本効率の改善(0.22)　Ｒ＝株主還元の強化(0.16)　Ｋ＝改革シグナル＝開示(0.17)　Ｅ＝実行の確かさ(0.13)　Ｑ＝ワナ耐性(0.12)　Ｐ＝割安のワナ減点。最も重いのは改善の実行(Ｃ)で、割安(Ｖ)より重い。決めた後に各重みを±20％動かしても顔ぶれが変わらないことを確かめた。",
        hitokoto="割安さより「改善を実行した証拠」を重くして、変われる会社を点数にする。")
...
fig("fig2_ha_v9.png", f"図表Ⅱ-7　スクリーニング2(破)の絞り込み ― 適格{F9['n_base']:,}社から変わる堀5社へ",
    "注：棒の長さは残った会社数(対数目盛)。変わる堀の点数(式10)は2-4の並べ替えにのみ使い、最終20社の配分には使わない。出所：選定記録より筆者作成。")
_j = DATA["9022"]
...
formula("11", "生まれる堀の点数(離の評価に使用・私たちの設計)",
        ...
        "Ｉ＝無形資産(重み0.18)　Ｎ＝技術力(0.15)　Ｂ＝急所を握る度合い＝価格決定力(0.18)　Ａ＝ＡＩ基盤への実需接続(0.22＝最重視)　Ｄ＝データ・顧客基盤(0.14)　Ｔ＝信頼・安全(0.13)　加点Ｂ^Evi＝証拠水準{1,2,3}を{2,4,8}点に対応づけ　減点Ｐ^Hype＝キーワードの接点しかない会社。",
        hitokoto="ＡＩの実需にどれだけ深く接続しているかを、証拠の強さも足して点数にする。")
body(f"重みは、中心仮説「ＡＩ基盤への実需接続が新しい堀を生む」からＡ(ＡＩ接続)を最重視し、堀の古典的源泉(無形資産・急所を握る度合い)と技術力が続く――過去リターンでは決めない。加点は証拠水準が1段上がるごとに2点→4点→8点と倍にした(水準3=数字まで確認はそれだけ希少)。ただし正直に書く――この点数の土台になる開示データのキーワード照合が前述のとおり飽和する({F9['ri']['keyword_path']['tie_n']}社が同点)ため、式(11)は選定の決定打にはならない。役割は二つに限る: 候補の点検・比較の物差しと、キーワードの接点しかない会社への減点(Ｐ^Hype)である。", after=2)
...
formula("12", "証拠の強さ(点数と分けて管理・私たちの設計)",
        ...
        "ＴＱ＝数字の改善で確認できる変わる証拠　ＴＳ＝株主還元の実行の証拠　ＴＲ＝改革の開示(計画文書)の証拠　ＥＭ＝新テーマ(生まれる堀)の開示証拠。読み方: min＝両方の証拠が必要(弱い方で判定)、max＝どれか一つ強ければ十分。水準1＝キーワードの接点のみ／水準2＝製品・顧客・投資計画の具体性を確認／水準3＝売上・受注・投資額の数字まで確認。なおＴＳ・ＴＲは今回のデータでは全社が同値となり判定に効いておらず、変わる堀の水準は実質ＴＱで決まる。",
        hitokoto="キーワードだけ＝水準1、製品・顧客の具体性＝水準2、数字まで確認＝水準3。")
_e3 = sum(1 for v in DATA.values() if v.get("evid") == 3); _e2 = sum(1 for v in DATA.values() if v.get("evid") == 2)
verdict(f"よって、点数がどれほど高くても、証拠の水準が足りない会社は役割の候補から外す。私たちが選定時に付けた水準は、水準3が{_e3}社・水準2が{_e2}社だった(守の完成した堀は品質基準で選び、証拠水準は破・離の判定に用いる)。")

# --- v11で追加: 自分たちの証拠水準を、成果物に照らして監査した(V11_PLAN §1のF) ---
_own_absent = len(_own["absent_from_candidate_pool_file"])
_own_lv = _own["thematic_level_distribution"]
banner3("自己監査 ― 私たちの「証拠主義」は、どこまで実行できていたか")
body(f"ここで、本レポートで最も都合の悪い事実を先に書く。上の水準は**選定時に私たちが付けた暫定値**であり、"
     f"生成スクリプトにも『証拠水準は暫定(要ＩＲ最終確認)』と但し書きが残っている。その最終確認を、私たちは"
     f"最後まで終えられなかった。そこで提出前に、20社それぞれについて**出典ＵＲＬと引用文が成果物として"
     f"残っているか**を機械的に照合した。結果は次のとおりである――ＵＲＬと引用まで揃っているのは"
     f"**{_own['n_hand_verified']}社({_own['hand_verified_holdings'][0]['name']})のみ**。"
     f"候補プールの記録に照らすと、主題(ＡＩ・半導体)の証拠水準は水準2が{_own_lv.get('2',0)}社、"
     f"キーワードの接点だけの水準1が{_own_lv.get('1',0)}社、接点なしの水準0が{_own_lv.get('0',0)}社、"
     f"そもそも候補プールの採点対象外だったものが{_own_absent}社だった。", after=2)
body(f"つまり――**私たちが掲げた「証拠主義」は、20社のうち{_own['n_hand_verified']}社でしか完遂できていない**。"
     f"残る19社の役割の根拠は、事業への一般的な理解に基づく叙述的な判断であって、開示資料を精読した結果ではない。"
     f"この差を「証拠に基づく選定」と書けば、実際にはやっていない作業を主張することになる。だから書き直す――"
     f"本レポートの選定は、機械的な関門と点数の上に、**成果物で裏づけた証拠1社分と、裏づけの弱い判断19社分**を"
     f"重ねたものである。第Ⅳ章の取材は、この空白を埋めるために設計した。", after=2)
body(f"これは第Ⅰ章で既製スコアを批判したのと同じ失敗である――あちらは業種ラベルを企業の実力と、こちらは暫定の自己申告を検証済みの証拠と取り違えていた。**内側からは気づけなかった**点まで同じである。", after=2)
...
body(f"結果: 候補を精査し、事業セグメントの開示でＡＩ・半導体・光通信の実需を確認できたのは{_ri[0]['n']}社(うち予備2社)。何をどの開示で確認したかを検証台帳として残す(下表)――誰でも同じ資料で追試できる。適格ガード(3-3)を通過した5社――{_rnames}――を生まれる堀とした。なお正直に書き添える――この5社のうちｓａｎｔｅｃ ＨＤ(6777)と日本マイクロニクス(6871)は、第Ⅲ章で比較する新バフェット型20社にも入っていた。私たちは財務の数字ではなく事業の開示から先にこの2社を選んだが、生まれる堀が財務数値にも現れ始めていたことになる。彼の式では選ばれないのは残る13社である。")
...
fig("fig2_ri_v9.png", f"図表Ⅱ-9　スクリーニング3(離)の分岐 ― キーワード経路を破棄し、事業の証拠で5社へ",
    "注：破棄した経路(3-1)も隠さず描く。事業検証(3-2)は開示資料に基づき、誰でも同じ資料で追試できる。出所：選定記録より筆者作成。")
_s = DATA["6777"]
...
body("なお、この構成が唯一の最適だという証明はできない。最適かどうかは未来の成績でしか分からないからだ。私たちが保証するのは、定義との整合と、次に示す壊れにくさである。")
fig("fig2_merge_v10.png", "図表Ⅱ-10　合流 ― 三世代の堀から最終20社へ(矢印は1対1の対応)",
    "注：守の5社は証拠の関所を通らず固定。両立型は二つの点数の両立を、証拠は厳しい方で判定する。筆者作成。", maxw=112)

banner3("選定は壊れにくいか ― 条件③の検証(要約)")
body(f"「複雑なルールを後から調整して、結論ありきで20社を選んだのではないか」という疑いには、三通りの検査で答えた(下表)。事前に宣言した目安(条件を一つ変えても大半が一致)を満たし、条件③は達成である。都合の悪い結果も書く――配分を最小分散に替えるとリターンは主対照を下回るが、その場合も効率(シャープ{_WV['最小分散']['3y']['sharpe']:.2f}対{C3['sharpe']:.2f})と下落({_WV['最小分散']['3y']['max_drawdown']:.1%}対{C3['max_drawdown']:.1%})では上回る。", after=2)
table(["何を動かしたか", "動かした通り数", "結果"],
      [[f"守の品質関門の目安・順位・業種上限", f"{ROB['n_variants']}通り",
        f"完成した堀5社は最小でも{ROB['min_overlap']}社が一致。核の4社(ＺＯＺＯ・日本オラクル・名村造船・サンリオ)は全通りで不変"],
       ["500万円の配り方(配分方式)", "3通り",
        f"均等{_WV['均等']['3y']['ann_return']:.1%}・役割予算(採用){_WV['役割予算(採用)']['3y']['ann_return']:.1%}とも主対照({C3['ann_return']:.1%})を上回る。最小分散{_WV['最小分散']['3y']['ann_return']:.1%}はリターンで下回るが下落は最小"],
       ["相場の局面(利上げ・ＡＩ相場・暴落)", "4局面",
        "下落が小さいのは4局面すべて。リターンは暴落局面を除く3局面で上回る(第Ⅲ章)"]],
      [46, 24, 100], fs=10)
para("図表Ⅱ-11　壊れにくさの検査(要約) ― 条件③の検証。数値は3年・年率。出所：筆者計算。", MIN, 10, after=5)
point("守で完成した堀5社(条件①)、破・離で15社(条件②)を選び、目安・配分方式・相場局面の三通りで壊れにくさ(条件③)を確かめた。")
...
body("どの銘柄が上がるかという予想は、一切使わない。予想を使えば、せっかく証拠で選んだ20社に、予想の当て推量が混ざってしまうからだ。配分は次の二段階だけで決めた。")
bullet("第一段階　役割ごとの予算", "20社を五つの役割に分け(前章)、役割ごとに予算を決める。完成・変化・新生の三世代に各28％、両立型10％、分散役6％――三世代へ同額を置くのは、どの未来にも賭けないという設計の続きである。")
bullet("第二段階　役割の中では等分", "同じ役割の中では、どの会社を厚くするかを決める材料を持たない(それを決めるには成績の予想が要る)。だから予算を社数で等しく割る。等分は素朴に見えて、予想を当てにする配分より実際には崩れにくい(ＤｅＭｉｇｕｅｌら, 2009)。買い方は、1株から買える単元未満株(金額指定)で目標比率どおりに配分する(式14)。")
formula("13", "役割予算つきの配分 ― 選定済み20社への配り方(DeMiguel, Garlappi and Uppal, 2009)",
        r"\omega_i = \frac{B_{r(i)}}{n_{r(i)}}",
        "Ｂ＝その役割の予算(守28／破28／離28／両立10／分散6％)　ｎ＝その役割の社数(5／5／5／3／2社)。結果として1社あたりの比率は、三世代の各社が5.6％、両立型が3.3％、分散役が3.0％になる。分散の考え方の土台はMarkowitz (1952)、等分の有効性はDeMiguel, Garlappi and Uppal (2009)による。",
        hitokoto="役割ごとの予算を、その役割の社数で等しく割るだけ。値上がりの予想は使わない。")
verdict("よって、配分に予想は入らない。役割の予算と社数だけで、20社の比率が機械的に決まる。")
formula("14", "単元未満株での配分(日本取引所グループ「単元株制度」)",
        ...
        hitokoto="500万円を目標の比率どおりに配るため、1株単位で買える前提で株数を計算する。")
verdict(f"よって、目標比率どおりに金額を割り当て、総予算500万円のうち{INVEST:,}円(使用率{INVEST/5_000_000:.1%})を投資に充てた。")
wmax = max(DATA.values(), key=lambda z: z["w"])
body(f"最終配分と20社の顔ぶれを、図表Ⅲ-1に一覧で示す（配分・事業概要・堀の偏差値を一表に集約）。役割の予算は守28／破28／離28／両立10／分散6％で、1社あたりの最大は{wmax['w']*100:.1f}％――どの1社にも6％以上を賭けていない。堀の偏差値は全上場企業の中での位置を平均50で表した値で、現在の堀＝いまの競争優位、未来の堀＝これから生まれる競争優位の強さを示す。")
prows = []
for role in order:
...
para("図表Ⅲ-1　最終ポートフォリオ20社（総予算500万円・単元未満株の目標配分）。役割: 守＝完成した堀／破＝変わる堀／離＝生まれる堀。堀の偏差値＝全上場の中での位置(平均50)。出所：選定記録より筆者作成。", MIN, 10, after=6)
...
body("配分・事業概要・堀の偏差値は図表Ⅲ-1に集約した。各社の証拠水準は図表Ⅱ-8と第Ⅱ章の判定に示した。", after=6)

banner2("3．パフォーマンス検証 ― 新バフェット型・市場と比べる")
body("本節では第Ⅰ章の条件④を検証する。物差しは分析の前に宣言する――①年率リターン ②ＴＯＰＩＸ超過 ③効率(シャープ) ④市場との連動度(β) ⑤最大の下落 ⑥市場超過の安定度(ＩＲ)の6つ、期間は「選定に使った3年」と「直近1年」。用語を先に一行で置き換える――β(ベータ)＝市場が1％動くとき何％動くか(1より小さければ市場より穏やか)、シャープ＝値動きの荒さ1単位あたりのリターン(高いほど効率的)、ＩＲ＝市場を上回る力の安定度、ｔ値＝その差が偶然では出にくいかの目安(おおむね2以上で「偶然とは考えにくい」)、最大の下落＝期間中に高値からいちばん下がった幅。")
body("この検証は「未来を当てた」検証ではない。会社を選んだのは2026年の財務データであり、それを過去3年に当てて成績を測っている。だから将来の成績の予測ではなく、選び方が過去の相場でどう振る舞ったかを自分で確かめる自己検証である。配分にも直近の値動きは一切使っていない(役割の予算を社数で割るだけ・式13)。")
banner3("比べる相手をどう作ったか ― 重みに未来の情報を入れない")
body(f"主対照は『新バフェット型』――同じ出発点から、バフェットの品質基準(高収益・堀・財務健全)だけで選び、割安順位を付けられた{F9['shu']['n_priceable']}社すべてである。ここで一つ、決めておかなければならないことがある。20社にいくらずつ配分した相手と比べるのか、である。会社の大きさ(時価総額)で配分するのが自然だが、「いつの大きさか」で答えが変わる。3年で株価が33倍になった会社は、いまの大きさで配分すると大きな比重を占める。しかし3年前に、その会社にその比重を置くことは誰にもできない。")
body("そこで私たちは原則を先に決めた――対照の配分に未来の情報(値上がりして膨らんだ後の大きさ)を入れない。主対照は期首(3年前)の大きさで配分する。これが当時実際に組めた相手である。同時に、いまの大きさで配分し直した相手(期末加重)も参考として併記する。こちらは値上がりした会社に事後的に重みが集まるため、私たちにとって最も厳しい仮想の対照になる。どちらか都合の良い方だけを見せることはしない。")
body("参考として『純正グレアム型』も置く――堀や質を見ず、割安さだけで選ぶとどうなるかを見るための比較用の20社(等金額)である。さらに市場平均としてＴＯＰＩＸ・日経平均を並べる。いずれも同じ規律で、後から入れ替えずに計算した。")
body("位置づけにも限定をつける。どの対照も『式だけの機械選定』であり、バフェット本人の運用成績ではない。彼の選び方を式にして日本株に当てはめた、いわば“仮想バフェット”である。確かめたいのは、その選び方に対して、私たちが加えた15社が成績をどれだけ改善したか、である。")
fig("cum_v10.png", "図表Ⅲ-2　累積リターンの比較(過去3年・期首=1)",
    "注：本ＰＦ(20社)・新バフェット型(期首加重＝主対照／期末加重＝参考)・純正グレアム型・ＴＯＰＩＸ・日経平均。ＴＯＰＩＸ連動指数1306は未調整の株式分割(2026-03-30)を×10補正、指数系列の欠測を前営業日で補完。価格は調整後終値(配当込み。日経のみ価格指数)。すべて2026年時点選定の自己検証。出所：価格データより筆者作成。", maxw=140)
_HL = "E8F2EC"   # 本ＰＦが上回るセルの塗り
...
_rows3 = [
    ["年率リターン", f"{O3['ann_return']:.1%}／{O1['ann_return']:.1%}", f"{C3['ann_return']:.1%}／{C1['ann_return']:.1%}",
     f"{_pm(D3)}／{_pm(D1)}pt", f"{E3['ann_return']:.1%}／{E1['ann_return']:.1%}",
     f"{O3['topix_ann_return']:.1%}／{O1['topix_ann_return']:.1%}", True],
    ["ＴＯＰＩＸ超過", f"＋{O3['excess_vs_topix']*100:.1f}／＋{G1:.1f}pt", f"＋{C3['excess_vs_topix']*100:.1f}／＋{C1['excess_vs_topix']*100:.1f}pt",
     f"{_pm(D3)}／{_pm(D1)}pt", f"＋{E3['excess_vs_topix']*100:.1f}／＋{E1['excess_vs_topix']*100:.1f}pt", "―", True],
    ["効率シャープ\n(高いほど良い)", f"{O3['sharpe']:.2f}／{O1['sharpe']:.2f}", f"{C3['sharpe']:.2f}／{C1['sharpe']:.2f}",
     f"{_pm(O3['sharpe']-C3['sharpe'],2)}／{_pm(O1['sharpe']-C1['sharpe'],2)}", f"{E3['sharpe']:.2f}／{E1['sharpe']:.2f}", "―", True],
    ["市場との連動度β\n(低いほど穏やか)", f"{O3['beta_vs_topix']:.2f}／{O1['beta_vs_topix']:.2f}", f"{C3['beta_vs_topix']:.2f}／{C1['beta_vs_topix']:.2f}",
     f"{_pm(O3['beta_vs_topix']-C3['beta_vs_topix'],2)}／{_pm(O1['beta_vs_topix']-C1['beta_vs_topix'],2)}",
     f"{E3['beta_vs_topix']:.2f}／{E1['beta_vs_topix']:.2f}", "1.00", False],
    ["最大の下落\n(小さいほど良い)", f"{O3['max_drawdown']:.1%}／{O1['max_drawdown']:.1%}", f"{C3['max_drawdown']:.1%}／{C1['max_drawdown']:.1%}",
     f"{(C3['max_drawdown']-O3['max_drawdown'])*-100:.1f}／{(C1['max_drawdown']-O1['max_drawdown'])*-100:.1f}pt小さい",
     f"{E3['max_drawdown']:.1%}／{E1['max_drawdown']:.1%}", f"{O3['topix_max_drawdown']:.1%}／{O1['topix_max_drawdown']:.1%}", True],
    ["超過の安定度ＩＲ\n(高いほど良い)", f"＋{O3['information_ratio']:.2f}／＋{O1['information_ratio']:.2f}", f"＋{C3['information_ratio']:.2f}／＋{C1['information_ratio']:.2f}",
     f"{_pm(O3['information_ratio']-C3['information_ratio'],2)}／{_pm(O1['information_ratio']-C1['information_ratio'],2)}",
     f"＋{E3['information_ratio']:.2f}／＋{E1['information_ratio']:.2f}", "―", True],
]
_t3 = table(["物差し\n(3年／直近1年)", "本ＰＦ\n(20社)", "主対照\n新バフェット型\n期首加重", "差\n本ＰＦ−主対照", "参考\n新バフェット型\n期末加重", "ＴＯＰＩＸ"],
            [r[:-1] for r in _rows3], [32, 25, 30, 27, 30, 24], fs=10, zebra=False)
for _i3, _r3 in enumerate(_rows3):        # 本ＰＦが上回った差のセルだけ塗る
    if _r3[-1]:
        X.shade_cell(_t3.rows[_i3 + 1].cells[3], _HL)
para("図表Ⅲ-3　対照群・市場との比較(各セル＝3年／直近1年)。塗ったセルは本ＰＦが主対照を上回った幅。3年は6つの物差しすべてで上回り、直近1年はβ(1.05対1.03)だけわずかに及ばない。日経平均は図表Ⅲ-2に併記した(年率32.8％／78.8％)。出所：筆者計算。", MIN, 10, after=5)
body(f"読み取りは三つ。第一に、当時実際に組めた新バフェット型(期首加重)に対して、本ＰＦは3年では6つの物差しすべてで上回り、直近1年もβを除く5つで上回った。直近1年の差は統計的にも確実である(日次超過のNewey-Westｔ値＝{SIG['1y']['ours_vs_buffett20_start']['t_newey_west']:.2f})。3年ではｔ＝{SIG['3y']['ours_vs_buffett20_start']['t_newey_west']:.2f}で、差の向きは一貫するが「偶然とは考えにくい」水準には届かない。グレアム型(ｔ＝{SIG['3y']['ours_vs_graham']['t_newey_west']:.2f})と市場ＴＯＰＩＸ(ｔ＝{SIG['3y']['ours_vs_topix']['t_newey_west']:.2f})には有意に上回った。第二に、都合の悪い結果も書く――同じ20社をいまの大きさで配分し直した仮想の対照(期末加重)には、3年のリターンで{-DE3:.1f}ポイント届かない。値上がりした半導体大手に事後的に重みが集まるためである。それでも効率(シャープ{O3['sharpe']:.2f}対{E3['sharpe']:.2f})・最大の下落({O3['max_drawdown']:.1%}対{E3['max_drawdown']:.1%})・連動度β({O3['beta_vs_topix']:.2f}対{E3['beta_vs_topix']:.2f})では上回る。値動きの荒さを対照とそろえた換算(約{LV3['leverage']}倍で運用)ではリターンも上回るが、借入を前提とする参考計算にすぎない。第三に、これはすべて自己検証であり、将来の市場超過の証明ではない――この限界は言い換えずに残す。")
banner3("対照の作り方を変えても結論は同じか")
body("「12社にしていれば勝てたのに、20社を選んだのは損な作り方ではないか」――逆に「都合の良い社数を選んだのではないか」という疑いにも、先に答えておく。社数(業種上限つき12社／同15社／全20社)と配分(期首・期末・等金額)の9通りをすべて計算した(下表)。")
_srows = []
for _n in ["12社", "15社", "20社"]:
    _r = [_n]
    for _lab in ["期首加重", "期末加重", "等金額"]:
        _v = SENS[f"{_n}_{_lab}"]["3y"]
        _r.append(f"{_v['ann_return']:.1%}（差{_pm(_v['ours_minus']*100)}pt）")
    _srows.append(_r)
table(["対照の社数", "期首加重(主系列の作り方)", "期末加重", "等金額"], _srows, [26, 52, 46, 46], fs=10)
para(f"図表Ⅲ-4　対照の作り方を変えたときの3年年率リターン(かっこ内は本ＰＦ{O3['ann_return']:.1%}との差)。出所：筆者計算。", MIN, 10, after=5)
body("読み取りは単純である。期首加重(未来の情報を入れない作り方)では、社数を12社・15社・20社のどれにしても本ＰＦが上回る。等金額でも12社・15社では上回り、20社では互角である。本ＰＦが届かないのは期末加重の15社・20社だけ――つまり「上がった後の大きさで配分し直す」という、当時は選べなかった作り方のときだけである。私たちは主系列として最も原則的な作り方(社数を切らない20社・期首加重)を採り、残りも隠さず開示する。")
banner3("四つの相場局面ではどうだったか")
_prows = []
for _pn, _lab in [("P1_2021-22利上げ", "利上げ相場 2021-06〜2022-12"), ("P2_2023-24AI前半", "ＡＩ相場の前半 2023-01〜2024-06"),
                  ("P3_2024-26直近", "直近 2024-07〜2026-06"), ("crash_2024-08暴落", "暴落ストレス 2024-07-25〜08-09")]:
    _p = PH[_pn]
    _prows.append([_lab, f"{_p['ours']['total_return']:+.1%}", f"{_p['control_buffett20_start']['total_return']:+.1%}",
                   f"{_p['control_buffett20_end']['total_return']:+.1%}",
                   f"{_p['ours']['max_drawdown']:.1%}／{_p['control_buffett20_start']['max_drawdown']:.1%}"])
table(["相場の局面", "本ＰＦ", "主対照\n(期首加重)", "参考\n(期末加重)", "最大の下落\n本ＰＦ／主対照"], _prows, [56, 24, 26, 26, 38], fs=10)
para("図表Ⅲ-5　相場局面別の期間リターン(株数を固定した買い持ちで計算)。局面の区切りは相場環境で先に定義し、成績を見て動かしていない。出所：筆者計算。", MIN, 10, after=5)
body("四つの局面のうち、リターンで主対照を上回ったのは三局面である(利上げ相場・ＡＩ相場前半・直近)。暴落の2週間だけはほぼ互角の下げ幅だった。一方、期間中の最大の下落は四局面すべてで本ＰＦのほうが小さい。三世代へ分散したことは、上げ相場で置いていかれない程度に攻めつつ、下げでは毎回いくぶん踏みとどまる、という形で効いていた。")
body(f"役割の分担も確かめた。過去3年の値上がり寄与は、生まれる堀(ＡＩ・半導体)が約{ROLE_CONTRIB['by_role_pct'].get('離 生まれる堀',0):.0f}％と牽引し、完成した堀(守)も約{ROLE_CONTRIB['by_role_pct'].get('守 完成した堀',0):.0f}％と土台以上に寄与、変わる堀(公益・資源)は約{ROLE_CONTRIB['by_role_pct'].get('破 変わる堀',0):.0f}％で下げ相場の緩衝材として効いた。旧来の割安一辺倒(グレアム型)に比べ、三世代へ寄与が分散している。なお本検証は取引コスト・税を考慮せず、毎日同じ比率へ戻す規約で計算した(局面別の表だけは株数固定の買い持ち)。買い方を変えると数値は多少動く。")

banner3("当時のデータだけで組んだらどうなったか ― 時点外検証(v11で新設)")
body("ここまでの比較には致命的になりうる弱点がある――**20社は2026年の情報で選び、それを過去に当てている**。"
     "「勝った」部分が選定の力なのか、後から振り返って選べたことの効果なのかを切り分けられない。"
     "そこで有価証券報告書の提出日時を使い、**基準日ごとに『その日までに提出済みの報告書』だけで機械的な関門を組み直して**、"
     "そこから前向きに測った。", after=2)
_pit_rows = []
for _a in PIT["as_of_dates"]:
    _f, _r, _m = PIT["funnels"][_a], PIT["results"][_a]["pool_eq"], CST["mde"][_a]
    _pit_rows.append([_a, f"{_f['passed_quality_gate']}社", f"{round(_r['days']/21)}カ月",
                      f"{_r['ann_return']*100:+.1f}%", f"{_r['bench_ann_return']*100:.1f}%",
                      f"{_r['excess_ann']*100:+.1f}pt", f"±{_m['mde_annualised']*100:.1f}pt"])
table(["基準日", "関門通過", "前向き期間", "関門通過プール\n(等金額)", "ＴＯＰＩＸ", "差", "検出できる差\nの最小値"],
      _pit_rows, [22, 18, 20, 24, 18, 18, 24], fs=10, fixed=True)
para(f"図表Ⅲ-7　時点外検証 ― 各基準日までに提出済みの報告書だけで関門を組み直した。出所：筆者計算({PIT['end']}まで)。",
     MIN, 10, after=6)
body(f"結果は私たちに厳しい。**関門の組み方を変えた{sum(len(PIT['results'][a]) for a in PIT['as_of_dates'])}通りすべてがＴＯＰＩＸに届かなかった**。"
     f"上位5社に絞っても改善せず、βはどの系列も1.1以下で上げ相場に乗り切れていない"
     f"(3期無赤字の関門は最も早い基準日では2期分しか報告書が無いため『入手できる期の無赤字』に読み替えた)。"
     f"ただし**「関門が価値を壊す」とも結論できない**――右端は検出できる差の最小値(検出力80%)で、3基準日のうち2つでは"
     f"観測された下振れがこれより小さい。**設計の分解能が足りない**。言えるのは狭い一点である――"
     f"**本ＰＦの過去成績を、機械的な関門の力に帰することはできない**。", after=2)

banner3("成績はどこから来たのか ― 市場と業種に分解する(v11で新設)")
_hd = FAC.get("headline_spec", "capm_sector")
_ao = FAC["regressions"]["ours_equal"][_hd]; _ab = FAC["regressions"]["benchmark_buf20_equal"][_hd]
_pm = PLA["percentiles"]["sector_matched"]["ours_equal"]
_co = CST["results"]["ours_equal"]
body(f"3年間の成績を市場と業種に分解した(月次{FAC['sample']['n_months']}カ月)。電気機器業種への連動が"
     f"{_ao['loadings']['SECTOR']:.2f}と大きく、半導体・電機の追い風に強く乗っていた。業種を差し引いてなお残る部分は"
     f"年率{_ao['alpha_annualised']*100:+.1f}%――ここまでは有利な数字である。しかし**同じ関門から裁量を入れずに作った"
     f"20社の同じ数字は{_ab['alpha_annualised']*100:+.1f}%で、本ＰＦより大きい**(試したどのモデルでも同じ向き)。"
     f"**残った部分は私たちの裁量が足したものではない**。関門が拠る品質(ＲＯＥの高低)自体の寄与も年率"
     f"{FAC['factor_summary']['QMJ']['annualised']*100:+.1f}%とほぼゼロで、時点外検証と符合する。", after=2)
body(f"無作為抽出とも比べた。同じ2026年ガードを通る{PLA['pool_size']:,}社から20社を{PLA['n_draws']:,}回選ぶと"
     f"(乱数の種{PLA['seed']})、業種構成をそろえた抽出で本ＰＦは上位{100-_pm['ann_return']:.1f}%に入るが、"
     f"**βは上位{100-_pm['beta_vs_topix']:.1f}%、最大の下落は下から{_pm['max_drawdown']:.1f}%とリスクの側でも端にいる**――"
     f"上位の一部は多くリスクを取った対価である。業種構成をそろえるだけで中央値は"
     f"{PLA['distribution']['unstratified']['ann_return']['p50']*100:.1f}%→"
     f"{PLA['distribution']['sector_matched']['ann_return']['p50']*100:.1f}%に上がり、差の多くは業種配分で説明できる。"
     f"なお月次で組み替え片道{int(CST['conventions']['one_way_bp'])}bpと単元未満株{int(CST['conventions']['odd_lot_bp'])}bpを"
     f"引いても3年年率{_co['monthly_headline']['ann_return']*100:.1f}%(引く前"
     f"{_co['daily_rebalance_no_cost']['ann_return']*100:.1f}%)で、回転が年{_co['annual_turnover_monthly']:.2f}回と"
     f"少なくコストは結論を動かさない。", after=2)
...
table(["「超える」の条件", "結果", "判定"],
      [["① 同じ土俵に立つ", "品質基準を式から再現し、その完成した堀5社を固定保有(第Ⅱ章)", "達成"],
       ["② 死角に届く", f"品質レンズが映さない13社を選定。うち2社は彼の式にも入る＝堀が数字に現れ始めた会社。ただし証拠水準は選定時の暫定値であり、出典ＵＲＬと引用まで成果物で裏づけられたのは{_own['n_hand_verified']}社のみ(第Ⅱ章の自己監査)", "部分達成\n(証拠の裏づけ\nは1社)"],
       ["③ 壊れにくい", f"目安{ROB['n_variants']}通り・配分3方式・相場4局面で検査。守5は最小{ROB['min_overlap']}/5一致(第Ⅱ章末)", "達成"],
       ["④ 総体で上回る", f"当時組めた新バフェット型(期首加重)に、3年は6つの物差しすべて・1年はβを除く5つで上回る(3年＋{D3:.1f}pt・1年＋{D1:.1f}pt。1年はｔ＝{SIG['1y']['ours_vs_buffett20_start']['t_newey_west']:.2f}で有意、3年は有意差なし)。ただし期末加重の仮想対照にはリターンで{-DE3:.1f}pt届かず、時点外検証では関門が{sum(len(PIT['results'][a]) for a in PIT['as_of_dates'])}通りすべて市場に届かない。業種を差し引いた残りも機械的対照の方が大きい", "自己検証の範囲で\n達成。ただし成績を\n関門にも裁量にも\n帰属できない"]],
      [28, 112, 30], fs=10, fixed=True)
para("図表Ⅲ-6　判定表 ― 四条件に対する現時点の答え(すべて2026年時点選定の自己検証)。", MIN, 10, after=6)
body(f"第Ⅰ章の投資仮説――三世代を束ねたポートフォリオは、堀の“現在”だけを測る選定を、効率と下落の小ささで上回る――は、"
     f"主対照に対して支持された。効率(シャープ)・下落・βのいずれも上回り、四つの相場局面すべてで下落は小さかった。"
     f"だが提出前に加えた三つの検証は、この結論の**射程**を大きく狭めた。①当時のデータだけで関門を組み直すと"
     f"{sum(len(PIT['results'][a]) for a in PIT['as_of_dates'])}通りすべてが市場に届かない。"
     f"②業種を差し引いた残りは、裁量を入れない機械的対照の方が大きい。③証拠主義は20社中"
     f"{_own['n_hand_verified']}社でしか完遂していない。", after=2)
body("したがって、私たちが正直に言えることはこうである――**過去3年の成績は事実として上回った。しかしその成績を、"
     "私たちの関門の力にも、私たちの裁量の力にも帰属させることはできなかった**。「バフェットを超えたか」への答えは、"
     "この一年の作業を経て「成績では上回ったが、なぜ上回ったのかを自分たちで説明できていない」に変わった。"
     "胸を張れるのは成績の大きさではなく、比べる相手を自分に都合よく作らなかったこと、"
     "そして**自分の設計が自分の基準を満たしていないと分かったときに、それを書き換えずに書き足したこと**にある。"
     "すべて2026年時点の選定を過去に当てた自己検証であり、将来の約束ではない。")

body(f"最後に、この設計の弱点を私たち自身の手で記録しておく――強みだけを並べたレポートは検証に耐えないからである。①テーマの偏り: 値上がり寄与の約{ROLE_CONTRIB['by_role_pct'].get('離 生まれる堀',0):.0f}%は生まれる堀(ＡＩ・半導体)に集中し、テーマの重みは約40%(業種の集中度ＨＨＩ＝0.16)。『ＡＩ・半導体バリューチェーンへの意図的な賭け』として隠さず開示する。テーマが崩れる未来では市場に劣りうるため、守・破に予算を置いて緩衝している。②単元未満株の前提: 株価の高い{N_UNBUY}社は通常の売買単位(100株)では¥500万に収まらず、本配分は単元未満株(金額指定)の取扱いを前提とした目標ウェイトである。③証拠は開示ベースで、しかも完遂できていない(第Ⅱ章の自己監査)――現場の実態までは測れず、第Ⅳ章の取材はこの限界を埋めるために設計した。④出発点は生存者のみ・⑤対照の作り方に結論が左右される・⑥当時のデータでは関門が市場に届かない――この三点は本章3の検証で数値とともに示した。⑦検出力の限界: 3基準日はプールが重なり独立した3回の観測ではない。差を検出する分解能そのものが足りていない。")
point(f"当時組めた新バフェット型(期首加重)には3年で6つの物差しすべて上回り、上がった後の大きさで配分した仮想対照にはリターンで届かない。ただし提出前に加えた三つの検証が結論の射程を狭めた――**当時のデータで関門を組み直すと{sum(len(PIT['results'][a]) for a in PIT['as_of_dates'])}通りすべて市場に届かず、業種を差し引いた残りは機械的対照の方が大きく、証拠主義は20社中{_own['n_hand_verified']}社でしか完遂していない**。成績は上回ったが、その理由を自分たちで説明できていない。")
...
# ================= 参考文献 =================
banner1("参考文献")
body("本文で引用した文献に加え、選定・検証・データ取得の設計で参照した文献を分野別に記載する。新バフェットの品質基準はBuffett's Alpha(Frazzini, Kabiller and Pedersen 2018)とQuality Minus Junk(Asness, Frazzini and Pedersen 2019)、割安×優良の複合順位はGreenblatt(2006)に依拠する。", after=3)
