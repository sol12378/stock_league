...
# WP3: 破の重みは最適化正典から自動転記(liquidity最大の全開示。ハードコード禁止)
PH2W = json.load(open(ROOT / "outputs/phase2_perfect_final_break/optimization/selected_phase2_solution_clean.json", encoding="utf-8"))["selected_weights"]
import csv as _csv
...
# WP0.5: 守の関門社数は正典funnel CSVから自動転記(ハードコード廃止・行対応を実装と一致させる)
_fn = {r["step"]: r for r in _csv.DictReader(open(ROOT / "outputs/phase1_final/phase1_final_screening_funnel.csv", encoding="utf-8"))}
FN = {k: int(_fn[s]["count_after"]) for k, s in [
    ("value_avail", "A_universe_value_liquidity_available"), ("value", "B_value"),
    ("quality", "C_quality"), ("financial", "D_financial_strength"),
    ("earnings", "E_earnings_quality"), ("distress", "F_distress"), ("liquidity", "G_liquidity")]}
FN_universe = int(_fn["A_universe_value_liquidity_available"]["count_before"])
FN_distress_removed = int(_fn["F_distress"]["removed_count"])
...
def table(hdr, rows, widths, hdr_fill=NAVY, fs=10, zebra=True, aligns=None, spacer=True, keep_caption=False):
...
body("本レポートの軸は一つだけである。目指す先が「三世代の堀」(何を買うか)、そこへ至る道順が「守破離」(どう選ぶか)であり、堀が主役で守破離は選ぶ手順の名前にすぎない。守では先行研究の式だけで完成した堀を5社に絞り、破では式を変えずに候補を1,200社へ広げ、離では変わる堀・生まれる堀を証拠の強さつきで測って最終20社を組んだ。選定後に成績を見て銘柄を入れ替える「後からの選び直し」はしていない(あとから成績の良い銘柄だけを拾えば、どんな選び方でも立派に見えるからだ)。その代わり、この選び方が条件を一つ変えたくらいでは崩れないことを、16通りの検査で確かめた(唯一、候補を極端に狭める検査だけが選定の背骨として効く――詳細は第Ⅱ章末)。")
para("※守破離(しゅはり)＝日本の芸道の修行段階。守=型を守る、破=型を破る、離=独自の境地へ。", MIN, 10, after=4)
body(f"結論を先に言う。私たちは「超える」を四つの条件で定義した。①バフェットと同じ土俵に立つ、②彼の式では選ばれない15社を同じ厳しさの証拠で選ぶ、③選定が壊れにくい――この三つの設計条件は本文で確かめたとおり達成した。残る一つ、④成績は、同じ出発点(3,099社)から守の式だけで選んだ対照群『純正バフェットポートフォリオ』を自作し、先に決めた物差しで比較して検証した。結果、選定に使った3年間で年{D3:.1f}ポイント、選定に使っていない直近1年でも年{D1:.1f}ポイント、対照群を上回り、④も達成した。ただし市場平均(ＴＯＰＩＸ)には直近1年で{abs(G1):.1f}ポイント負けている――この事実も隠さず記し、市場平均へ勝ち続ける力の証明は来年の宿題として残す。")
...
X.setfont(p_h.add_run("完成した堀・変わる堀・生まれる堀を証拠で選んで一つに束ねたポートフォリオは、堀の“現在”だけを測る純正バフェット型の選定を、リターンと超過の安定度(ＩＲ)で上回る。守り(値動きの穏やかさ・最大の下落)ではむしろ純正型が優ることを認める――それこそが、完成した堀を全体の四分の一だけ残す理由である。"), MIN, 10.5, True, BLACK)
...
table(["条件", "内容", "どこで確かめるか"],
      [["① 同じ土俵に立つ", "バフェットの選び方を先行研究の式だけで再現し、その方法が選ぶ5社を実際に持つ。比較相手を自分のポートフォリオの中に持ち歩く", "第Ⅱ章「守」"],
       ...
       ["③ 壊れにくい", "選定が一つの仕掛け・一つのテーマに依存しない。合格ライン=条件を一つ変えても20社中11社以上が維持される(候補の広さの変更は別扱い)", "第Ⅱ章末の検査"],
       ["④ 成績で上回る", "対照群『純正バフェット』(守の式だけの20社)に対し、主の物差し=年率リターンと超過の安定度ＩＲで両期間とも上回る。参考=β・最大の下落(小さいほど守りが堅い)とＴＯＰＩＸ・日経は併記", "第Ⅲ章の検証"]],
      [34, 102, 34], fs=10)
...
body("守=条件①、離=条件②、章末の16通り検査=条件③、第Ⅲ章の対照群比較=条件④に対応する(下表)。物差しは比較の前に宣言する。")
table(["段階", "選ぶ堀とやること", "どこまでが借り物か", "検証する条件"],
      [["守", "完成した堀5社を、先行研究の式だけで選ぶ", "式も使い方も、すべて先行研究のまま", "条件①"],
       ["破", "堀はまだ選ばず候補1,200社を用意する(式を変えず使い方だけ調整)", "式は借り物。使い方は私たちの判断(過程を開示)", "(離の準備)"],
...
body("なお、本文の数字はすべて選定の過程で保存した記録(通過社数・除外理由・検査結果)から機械的に転記し、レポート自体を記録データから自動生成している。生成・検証コードと正典データ・乱数シード(探索42〜44・監査45)は提出時点で保存しており、どの数字も出どころまで遡って確かめられる。")
# (v6: SDGs記入欄はユーザー指示で削除。公式様式で必須の場合は復活させること)

# ================= Ⅱ スクリーニング =================
banner1("Ⅱ．スクリーニング ― 守・破・離", newpage=True)
body("前章では「超える」を四つの条件で定義し、検証の順番として守破離を置いた。本章が選定の中核である。3,099社の非金融普通株から、三段階のスクリーニングで最終20社を選ぶ。各段階では、何を見たか(観点)、何で測ったか(データと式)、どこで区切ったか(合格ライン)、何社が残ったか(結果)を必ず示す。データの基準は、価格が2023年4月25日〜2026年6月1日の日次、財務・開示がＥＤＩＮＥＴ提出書類(2026年7月8日取得。各社は取得日時点の直近有価証券報告書で、決算期の違いにより最大12カ月の鮮度差がある)である。")
...
table(["観点", "使うデータ・式", "合格ライン", "残った社数"],
      [["割安か(純資産)", "Ｂ／Ｍ＝純資産÷時価総額(式1)", "市場の上位30％", f"{FN_universe:,}→{FN['value_avail']:,}社(算出可能)"],
       ["割安か(利益)", "Ｅ／Ｐ＝純利益÷時価総額(式2)", "黒字かつ上位50％", f"→{FN['value']:,}社(割安2関門を通過)"],
       ["収益力は本物か", "粗利÷総資産(式3)", "市場の中央値以上", f"→{FN['quality']:,}社"],
       ["財務は健全か", "6項目チェックの合格割合(式4)", "0.65以上", f"→{FN['financial']:,}社"],
       ["利益は本物か", "利益と現金の差(式5)", "悪い側の上位30％を除外", f"→{FN['earnings']:,}社"],
       ["危険はないか", "債務超過・2期連続赤字(式6)", f"該当ゼロ(除外{FN_distress_removed}社)", f"→{FN['distress']:,}社"],
       ["実際に買えるか", "60日平均の売買代金(式7)", "基準1,000万円(300万〜1,000万は要確認で通過)", f"→{FN['liquidity']:,}社"]],
      [32, 52, 38, 48], fs=10)
...
verdict("よって、6項目の合格割合が0.65以上(6項目では実質4項目以上＝4/6≒0.67)の企業を「財務が健全」と評価する。")
...
body("いくら割安で優良に見えても、財務危機の恐れがある会社は長期保有に向かない。倒産予測の古典であるAltman (1968)のＺスコアとOhlson (1980)のＯスコアは、財務比率から倒産確率を推定するが、どちらも多数の係数推定を要する。私たちは両モデルが最も強い危険信号として共有する二つの極端な状態――債務超過(純資産がマイナス)と持続的な赤字――だけを取り出し、係数推定を伴わない除外条件として実装した(Altman, 1968；Ohlson, 1980に基づく安全条件)。", after=2)
formula("6", "危険よけ ― 除外の条件(Altman, 1968; Ohlson, 1980に基づく)",
        r"\mathrm{Exclude}_i = \mathbf{1}\left[\ \mathrm{Equity}_i \le 0\ \lor\ (\mathrm{NI}_{i,t}<0 \land \mathrm{NI}_{i,t-1}<0)\ \right]",
        "Equity＝純資産　ＮＩ＝純利益。純資産がマイナス(債務超過)、または2期連続の純損失に該当したら除外し、該当しない企業だけを通す。")
verdict("よって、債務超過・2期連続赤字に該当しない企業を「長期保有に耐える」と評価する。")
body(f"なお、この関門で新たに除外された企業はゼロだった(除外{FN_distress_removed}社)。割安の2関門(式1・式2)が純資産プラス・黒字を前提とするため、債務超過や連続赤字の企業はこの段階までに既に落ちているからである。関門6は独立の除外を生まないが、「すでに安全な集団」であることを式で確かめる念のための関所として置いている。", after=2)
...
formula("7", "60日平均売買代金 ― 売買のしやすさ(Amihud, 2002の基礎量に基づく)",
...
body("守の七関門を通過した77社は、破の出発点であり、後の比較の基準線でもある。ただし基準線となる完成した堀Top5には、七関門よりさらに厳しい条件を課した――全指標がそろい、売買代金1日1,000万円以上を(“要確認”でなく)明確に満たし、際立った異常値がなく、粗利÷総資産が業種内でも中央値以上、という条件で33社に絞った(選定フローの全段は技術補遺に記録)。この33社を、収益力→利益÷時価総額→純資産÷時価総額→6項目→利益の質→買いやすさ→会社の大きさの固定順に並べ、同一業種は原則2社までとして上位5社を固定した。選ばれたのは、"
     + "、".join(f"{d['code']} {JPN[d['code']]}({SEC_JP[d['sector']]})" for d in b5)
     + "の5社である。以降、この5社は一度も入れ替えない。")
fig("fig2_shu_v5.png", "図表Ⅱ-3　守の絞り込み ― 3,099社から完成した堀Top5へ",
...
body("守で使った7指標(割安2つ・収益力・6項目・利益の質・危険よけ・売買のしやすさ)は単位がばらばらで、そのままでは足し合わせられない。そこで各指標を「市場の中で下から何番目か」という順位の割合(0〜1)に置き換える。これは資産価格研究がＢ／Ｍなどの指標で銘柄を上位30％・中位40％・下位30％のように順位で区切って比べる慣行(Fama and French, 1993 ほか)を、区切りではなく連続的な順位点として使うものである。守の各関門(上位30％・上位50％・中央値以上)も同じ順位の物差しの上にあり、破は新しい物差しを持ち込んでいない。", after=2)
formula("8", "順位化 ― 市場の中の相対的な位置に置き換える(Fama and French, 1993 の順位で区切る慣行)",
...
body("7つの順位点に重みを付けて足し合わせ、危険な兆候(異常値・データ欠け・極端に小さい会社・一時的な利益)には減点を与える。複数のシグナルを一つの合成点に束ねる形は、9項目の合計点で勝ち組と負け組を分けたPiotroski (2000)のＦスコアと同型であり、私たちはその「合計」を「重みつき合計−減点」に一般化した。重み・減点の強さ・候補数・業種調整の有無・データ欠けの扱いという設定の選択は、無数の組合せを機械的に試すランダムサーチ(Bergstra and Bengio, 2012)ほか3系統で比較し、候補集合の効用(広さ・質・安全・業種分散など)が最も高くなる設定を採用した(採用後に順位の安定性を別途監査した。目的関数と探索の詳細は技術補遺§C・§E)。採用した設定は「市場順位で0〜1化・主要指標が欠けた会社は除外・業種調整はしない」である。", after=2)
formula("9", "合成点 ― 候補づくりのための重みつき合計(Piotroski, 2000 の合成の一般化。最終選定には使わない)",
...
body(f"重みについて、一つ正直に書き残す。最も大きい重みは売買のしやすさの{PH2W['liquidity']:.3f}で、次いで危険よけの{PH2W['distress']:.3f}である(7指標の全重みは技術補遺§C)。危険よけは、これを外しても結果はほとんど変わらない(目的関数の低下は0.0017)。危険な会社は合成点より前の段階で機械的に除外済み(財務除外184社+債務超過11社)であり、重みは「除外をすり抜けた残りの微調整」を担っているにすぎない。重みの大きさを予測力と読んではいけない――この多層防御の構造ごと開示しておく。")
banner3("手順3　順位の付け方を変えても、結果は同じか ― 4通りの監査")
body("0〜1化のやり方は一つではない。市場全体での順位・業種の中での順位・外れ値に強い標準化・端を丸めた標準化の4通りで順位を付け直し、顔ぶれが入れ替わらないかを確かめた。1,200社のうち970社は4通り中3通り以上で共通して上位に残り(中核。4通り全てで一致するのは554社)、2通り以上まで広げれば1,135社が安定していた。付け方に敏感な29社には確認フラグを付け、離の個別確認へ引き継いだ。")
banner3("手順4　候補は何社が適切か ― Top1200の採用")
body("広さ・質・安全・売買のしやすさ・業種の分散・人の目で確認できる規模を比べると、広さを重視するほどTop2000が有利になる。しかし目的は候補数の最大化ではなく「一社ずつ確認できる候補づくり」なので、Top1200を正式採用し、Top2000は取りこぼし確認用の参照群とした。Top100・Top300は日々の確認を優先する内側の層で、のちの壊れにくさ検査(候補を100社・300社に絞ったら選定がどう変わるか)の比較にも使う。この入れ子の関係を図表Ⅱ-5に示す。採用したTop1200は業種の偏りがなく(業種の集中度ＨＨＩ＝構成比の二乗和が0.0707・最大業種11.6％)、守の5社を全員含む(ただし2024年の財務断面で選び直すと守5社のうち4社に減る。年度別の検証は技術補遺§E)。")
table(["観点", "内容", "結果"],
      [["順位化(式8)", "各指標を市場の中での順位(0〜1)に置き換える。式の定義は変えない", "単位の違う7指標を公平に比較"],
       ["合成点(式9)", "重み・減点・候補数などをランダムサーチで比較し、効用が最大の設定を採用(採用後に安定性を監査)", "市場順位・欠損は除外・業種調整なし"],
...
fig("fig2_ha_v5.png", "図表Ⅱ-5　破 ― 候補リストの形成(Top1200を正式採用)",
    "注：破の点数は候補選びの道具であり、最終選定には使わない。筆者作成。")

step_chip(3, "離 ― 変わる堀・生まれる堀を測り、20社を組む")
body("離で初めて、私たち独自の物差しを導入する。ただし新しい式をゼロから発明したのではない。式(10)〜(12)は私たちの独自式だが、その導出は次の三つの原則に従う。第一に、部品は先行研究と守の式の再利用――測る量そのものは、割安(式1・式2)・利益の質(式5)・財務シグナル(式4系)・無形資産の文献に既にあるものだけを使う。第二に、重みは成績で決めない――どの重みなら過去の成績が良かったか、という決め方は後出しの罠なので、重みは投資テーマの論理から概念的に設計し、決めた後に「重みを揺らしても順位が壊れないか」を監査する。第三に、点数と証拠の分離――点数(魅力の大きさ)は連続値で設計の裁量が入るが、証拠(裏づけの強さ)は開示資料まで遡って誰でも確認できる。この二つを混ぜず、証拠を式(12)の独立した関所にする。以下、式ごとに「なぜこの形か」を説明する。")
banner3("式(10)の導出 ― 変わる堀は「割安×改善の実行」の因数分解")
body("変わる堀の定義は「割安に放置されているが、資本効率改革の実行によってこれから評価が変わる会社」である(第Ⅰ章)。この定義を測れる部品に因数分解すると、(a)いま割安か=Ｖ、(b)良くなっているか=資本効率の改善Ｃ、(c)その改善が本物か=還元の余力Ｆ・実行の裏づけＸ・利益の質Ｑ、(d)データは信頼できるか=Φ、そして(e)「安いだけの会社」を弾く減点=Ｐ、となる。Ｖは式(1)(2)の順位点、Ｑは式(5)、Ｘは式(4)系のシグナルの合成であり、測る量は守と同じ部品の再利用である。", after=2)
formula("10", "変わる堀の点数(選定に使用・私たちの設計)",
        r"S^{\mathrm{Trans}}_i = 0.22\,V_i + 0.24\,C_i + 0.10\,F_i + 0.18\,X_i + 0.16\,Q_i + 0.10\,\Phi_i - P^{\mathrm{Trap}}_i",
        "Ｖ＝割安さ(式1・2の順位点)　Ｃ＝資本効率の改善　Ｆ＝還元の余力(本業の現金収入−設備投資)　Ｘ＝実行の裏づけ(改善が財務の質に表れているか)　Ｑ＝利益の質(式5系)　Φ＝データの信頼度　Ｐ＝割安のワナ減点。結果は0〜100点に丸める。")
body("重みの決め方: 「変わる」の中心は改善の実行なのでＣを最大(0.24)に、土台の割安Ｖ(0.22)がそれに次ぎ、改善が本物かを確かめるＸ+Ｑ(合計0.34)を厚くし、Ｆ・Φは補助(各0.10)とした。この配分は過去リターンへの当てはめでは決めていない。その代わり、各重みを±20％動かしても順位の相関が0.9973を下回らず、選ばれる5社が変わらないことを事後に監査した(技術補遺§E)。減点Ｐは「安いだけの会社」の既知の型――利益に現金の裏づけがない・財務が危険・異常値・本業の現金収入がマイナス・赤字が続く等の8つの型――を列挙して点数から差し引く方式で、各項の点数と全社への適用結果は技術補遺§Bに全開示した。", after=2)
verdict("よって、割安さに「変わる証拠」が重なる会社ほど高得点とし、変わる堀の上位5社を選ぶ。安いだけの15社は減点で弾いた。")
banner3("式(11)の導出 ― 生まれる堀は「堀の源泉」を未来方向へ写像する")
body("第Ⅰ章でバフェットの堀を、ブランドや特許などの無形の強み・乗り換えの手間・利用者が増えるほど価値が増す仕組み・圧倒的なコスト優位、と分解した。生まれる堀は「これらがこれから形成される力」であり、会計数値に表れにくい。そこで、無形資産が企業価値の主役になったことを示す文献(Lev and Gu, 2016；Peters and Taylor, 2017)と、研究開発投資が株式リターンに結びつくことを示した実証(Chan, Lakonishok and Sougiannis, 2001)に沿って、堀の源泉を六つの測れる部品へ写像した――無形資産Ｉ・技術力Ｎ・急所を握る度合いＢ(価格決定力)・ＡＩ基盤への実需接続Ａ・データ顧客基盤Ｄ・信頼と安全Ｔである。", after=2)
formula("11", "生まれる堀の点数(選定に使用・私たちの設計)",
        r"S^{\mathrm{Emerg}}_i = 100\,(w_I I_i + w_N N_i + w_B B_i + w_A A_i + w_D D_i + w_T T_i) + B^{\mathrm{Evi}}_i - P^{\mathrm{Hype}}_i",
        "Ｉ＝無形資産(w=0.18)　Ｎ＝技術力(0.15)　Ｂ＝急所を握る度合い(0.18)　Ａ＝ＡＩ基盤への接続(0.22)　Ｄ＝データ・顧客基盤(0.14)　Ｔ＝信頼・安全の基盤(0.13)　加点Ｂ^Evi＝証拠水準{1,2,3}を{2,4,8}点に写像　減点Ｐ^Hype＝キーワードの接点のみ(18点)。結果は0〜100点に丸める。")
body("重みの決め方: 本レポートの中心仮説は「ＡＩ基盤への実需接続が新しい堀を生む」なので、Ａを最大(0.22)に置き、堀の古典的源泉であるＩ・Ｂ(各0.18)、技術力Ｎ(0.15)が続く。加点は証拠水準が1段上がるごとに2点→4点→8点と倍にした――水準3(数字まで確認)はそれだけ希少だからである。減点18点は、キーワードの接点しかない会社が他の部品点をどれだけ積んでも上位5社に届かない水準として設計し、実際に577社をこの減点と関所で排除した。全重み・減点の一覧と適用結果は技術補遺§Bに全開示した。", after=2)
...
verdict("よって、点数がどれほど高くても、証拠の水準が足りない会社は役割の候補から外す。最終20社の内訳は水準3が15社・水準2が4社・水準1が1社(水準1は守の固定枠4350メディカルシステムネットワーク。守の選定は証拠水準を使わない)。")
body("当てはめ例: 両立型のゼンリン(9474)は「変わる証拠」と「新テーマの証拠」の弱い方(min)で判定して水準3――どちらの裏づけも数字まで確認できた、という意味になる。", after=4)
body(f"候補{EXC['pool']:,}社から{EXC['selected']}社へ絞る過程で除外した{EXC['total_excluded']:,}社は、理由別にすべて記録した。うち{EXC['audited_subtotal']}社は個別に精査した5分類、残る{EXC['categories']['score_below_cutoff']}社は合成点が精査ラインに届かず一括の理由コードで記録した。選ばなかった理由を残すことが、選んだ理由の裏づけになる。")
...
table(["除外の理由", "社数", "説明"],
      [["ＡＩ関連の接点がキーワードのみ", f"{EXC['categories']['ai_keyword_only']}社", "機械的なキーワード照合でしか接点を確認できず、製品・顧客・数量まで遡れる開示証拠(水準2以上)を確認できなかった"],
       ["より良い類似候補あり", f"{EXC['categories']['already_represented_by_better_candidate']}社", "同じ業種・テーマに、証拠も点数も上回る候補が存在する"],
       ["財務が危険", f"{EXC['categories']['distress_or_quality_risk']}社", "債務超過・連続赤字など、長期保有に耐えない"],
       ["割安のワナ", f"{EXC['categories']['value_trap_risk']}社", "安いだけで、改善の実行が伴わない"],
       ["安さの根拠のみ", f"{EXC['categories']['low_pbr_only']}社", "ＰＢＲの低さ以外に変わる証拠がない"],
       ["合成点が精査ラインに未達", f"{EXC['categories']['score_below_cutoff']}社", "上の5分類の個別精査に進まず、一括の理由コードで記録"]],
      [42, 18, 110], fs=10)
para(f"図表Ⅱ-6　除外{EXC['total_excluded']:,}社の内訳(全社を理由つきで記録)。出所：exclusion_record_v5.csv(除外記録の正典)。", MIN, 10, after=6)
...
body(f"「バフェットが選ぶはずの5社は、本当に必要なのか」という問いには、検査で答える。基準線5社の固定を外して選び直す検査(章末の16通り検査の一つ・技術補遺§EのA11)では、最終20社のうち11社しか一致せず、完成した堀の5社は5社とも脱落する。つまりこの5社は、新しい物差しの高得点者だから残っているのではなく、比較相手として意図して固定している。隠すのではなく、それ自体を設計として明示する。なお、この構成が唯一の最適だという証明はできない。最適かどうかは未来の成績でしか分からないからだ。私たちが保証するのは、定義との整合と、次に示す壊れにくさである。")
fig("fig2_ri.png", "図表Ⅱ-7　離 ― 三世代の堀から最終20社へ(矢印は1対1の対応)",
    "注：守の5社は証拠の関所を通らず固定。両立型は二つの点数の両立を、証拠は厳しい方で判定する。筆者作成。")

# v6: 16通り検査の詳細(図表・4家族表)はユーザー指示で本編から削除し要約のみ残す。全表は技術補遺§E。
ABL_OV = {r["variant"]: int(r["overlap_with_final20"]) for r in ABL}
banner3("選定は壊れにくいか ― 条件③の検証(要約)")
body(f"「複雑なルールを後から調整して、結論ありきで20社を選んだのではないか」という疑いには、条件を一つずつ変えた16通りの選び直しで答えた。結果、候補を100社に狭める検査(A8)だけが{ABL_OV['A8']}社まで崩れ、残る15通りは{min(v for k, v in ABL_OV.items() if k != 'A8')}社以上が一致した――事前に宣言した合格ライン(20社中11社以上)を満たし、条件③は達成である。最も効くのは候補の広さであり、破の「広く作って確認する」という判断がここで裏づけられる。16通り全ての内容・一致数・Jaccard係数は技術補遺§Eに全表を記録した。", after=2)
point("本章の要点: 守で基準線5社(条件①)、離で死角を補う15社(条件②)を選び、16通りの検査で壊れにくさ(条件③)を確かめた(全表は技術補遺§E)。")
...
def role_fact(d):
    ...
    if r == "Transformation Core":
        return (f"変わる堀の点数{d['tsc']:.0f}点(20社中の変わる堀5社)。改善・還元の証拠(証拠の強さ "
                f"水準{d['evid']})を持つ(純資産比{d['bm']:.2f}倍)。比率{d['w']*100:.1f}％")
    if r == "Emerging Core":
...
body("前節までで20社と配分が決まった。本節では第Ⅰ章の条件④を検証する。比べる物差しは、分析の前に宣言しておく――①年率リターン ②ＴＯＰＩＸ超過 ③市場との連動度(β) ④最大の下落 ⑤市場超過の安定度(ＩＲ)の5つ、期間は「選定に使った3年」と「選定に使っていない直近1年」に分ける。正確には、この直近1年も選定の点数計算に使っていないという意味であり、配分の値動き係数(式13のσ)は同じ1年の変動を入力に使う――完全な時点外検証ではないと限定をつけて読む(技術補遺§G)。過去データで選ぶ条件はどのチームも同じであり、その分は割り引いて読む――この注意はここに一度だけ書く。")
...
body("対照群の位置づけに、先に限定をつける。『純正バフェット』はあくまで式だけの機械選定であり、バフェット本人の運用(少数への集中・保険料を使った資金調達・数十年の保有)そのものではない。守の式は割安×優良の中小型株に寄るため、ＡＩ主導の大型グロース株が牽引したこの3年の相場では、市場平均に劣後しやすい――実際、対照群はＴＯＰＩＸに大きく劣後している(図表Ⅲ-8)。したがってこの比較は「バフェット本人に勝った」ことを意味せず、「同じ出発点で式だけを使った場合に対し、三世代の設計が何を足したか」を測る基準線として読む。")
fig("fig3_cum_v5.png", "図表Ⅲ-7　累積リターンの比較(過去3年・期首=1)",
    "注：ＴＯＰＩＸ連動指数1306は未調整の株式分割(2026-03-30)を補正し、指数系列の欠測1日(2025-10-24)を前営業日で補完済み。価格はyfinance調整後終値(配当込み。日経平均のみ価格指数)。出所：control_comparison_v5.json・価格正典データ。")
table(["物差し", "本ＰＦ", "純正バフェット(対照)", "ＴＯＰＩＸ", "日経平均"],
      [["年率リターン(3年)", f"{O3['ann_return']:.1%}", f"{C3['ann_return']:.1%}", f"{O3['topix_ann_return']:.1%}", f"{O3['nikkei_ann_return']:.1%}"],
       ...
       ["市場との連動度β(直近1年)", f"{O1['beta_vs_topix']:.3f}", f"{C1['beta_vs_topix']:.3f}", "1.000", "―"],
       ["最大の下落(直近1年)", f"{O1['max_drawdown']:.1%}", f"{C1['max_drawdown']:.1%}", f"{O1['topix_max_drawdown']:.1%}", "―"],
       ["超過の安定度ＩＲ(直近1年)", f"{O1['information_ratio']:.3f}", f"{C1['information_ratio']:.3f}", "―", "―"]],
      [40, 30, 38, 28, 28], fs=10)
para("図表Ⅲ-8　対照群・参照ベンチマークとの比較(5つの物差し×2期間)。日経平均は値がさハイテク株の比重が高くＡＩ相場で大きく伸びやすい参照値(配当抜き)。出所：control_comparison_v5.json(phase5と同一の計算規約、指数系列の欠測は補完済み)。", MIN, 10, after=6)
body(f"読み取りは三つ。第一に、本ポートフォリオは対照群を選定に使った3年間で年{D3:.1f}ポイント、選定に使っていない直近1年でも年{D1:.1f}ポイント上回った――守の式だけでは届かない部分を、変わる堀・生まれる堀が実際に運んだ。この超過は3年で統計的にも有意(日次超過ｔ値＝{SIG['3y']['ours_vs_control']['t_newey_west']:.2f}。直近1年は{SIG['1y']['ours_vs_control']['t_newey_west']:.2f}で10％水準、技術補遺§F)。第二に、正直に認める点として、守りは対照群の方が堅い(最大の下落も市場との連動度も対照群が小さい)。第三に、参照のＴＯＰＩＸには直近1年で年{G1:.1f}ポイント劣後し、日経平均には3年でも届いていない。ＴＯＰＩＸ超過は3年でもｔ値＝{SIG['3y']['ours_vs_topix']['t_newey_west']:.2f}で有意といえず、対照群には勝ったが、市場平均に勝ち続ける力までは証明できていない――ここは言い換えずに残す。")
body(f"役割の分担も確かめた。過去3年の値上がり寄与の8割は生まれる堀(フジクラ・オルガノなど)で、完成した堀は寄与1％の安定した土台に徹していた。また、割安側の銘柄を外して組み直すと値動きの大きさが35.5％へ悪化する。変わる堀は攻めではなく、揺れを抑える緩衝材として効いている。なお本検証は取引コスト・税を考慮せず、毎日同じ比率へ戻す固定重み規約で計算した。株数固定の買い放置では値が変わり下落も深い(本ＰＦ3年{BH['ours']['3y']['ann_return']:.1%}・下落{BH['ours']['3y']['max_drawdown']:.1%}、技術補遺§D)。")
...
table(["「超える」の条件", "結果", "判定"],
      [["① 同じ土俵に立つ", "守で規律を式だけから再現し、その5社を固定保有(第Ⅱ章)", "達成"],
       ["② 死角を補う", f"バフェットの式では選ばれない15社を、証拠の強さつきで選定。最終20社の証拠内訳は水準3=15社・水準2=4社・水準1=1社。除外{EXC['total_excluded']:,}社は理由つきで全記録(第Ⅱ章)", "達成"],
...
body(f"四つの条件が、それぞれの持ち場で達成となった。①同じ土俵・②死角の補完・③壊れにくさは設計と検査で、④成績は主の物差し(年率リターン・超過の安定度ＩＲ)が両期間とも対照群を上回った。守り(値動きの穏やかさ・最大の下落)ではむしろ純正型が優ったが、これは弱点ではなく、完成した堀を四分の一残した設計が守りを担う裏返しである。ＴＯＰＩＸへの直近1年{abs(G1):.1f}ポイントの劣後という宿題を隠さないことも、設計の一部である。第Ⅰ章の投資仮説――三世代を束ねた選定は、リターンと超過の安定度で純正バフェット型を上回る――は、この比較によって支持された。")
body(f"最後に一つ。証拠水準の分離管理・除外{EXC['total_excluded']:,}社の全記録・16通りの壊れにくさ検査――この三つは、既製の手法ではなく、私たちが検証可能性のために自分の手で組んだ、本レポート固有の装備である。私たちがこの20社を持ち続けられる理由は、成績の自信ではなく、この構造と証拠と検査にある。")

banner2("5．リスクと限界 ― 正直に書き残す")
body("最後に、この設計の弱点を私たち自身の手で記録しておく――強みだけを並べたレポートは検証に耐えないからである。")
bullet("テーマの偏り", f"過去3年の値上がり寄与の8割は生まれる堀に集中しており、テーマ寄与の集中度(ＨＨＩ)は{PERF['hhi']['theme']:.2f}と高め(業種の集中度0.0707とは別の物差し)。生まれる堀のテーマが崩れる未来では、市場に劣る可能性がある。だからこそ完成した堀・変わる堀に同額の予算を置き、一つの未来への賭けを避けている。")
...
bullet("出発点は生存者のみ", f"出発点の{FN_universe:,}社は2026年4月30日時点の上場一覧で、期間中に上場廃止となった企業を含まない(生存者バイアス)。成績はその分有利に出うる(技術補遺§A)。")
...
# ================= 参考文献 =================
banner1("参考文献")
body("本文で直接引用した文献に加え、選定・最適化・検証・データ取得の設計にあたって参照した文献を、分野別にすべて記載する(正典: docs/references_master.md)。なお最適化は3系統(ランダムサーチ・ＴＰＥ・ＮＳＧＡ-Ⅱ)を実際に比較しランダムサーチの最良解を採用した——掲載する最適化系文献にはこの比較検討で参照した代替手法を含む。", after=5)
