...
out = {
    ...
    "panel_totals": {
        "filings": int(len(fund.dropna(subset=["submit_date"]))),
        "firms": int(fund.dropna(subset=["submit_date"]).code.nunique()),
        # fiscal years ON FILE PER FIRM (not distinct calendar years, which spans more because
        # fiscal year-ends fall in different months)
        "median_fiscal_years_per_firm": int(
            fund.dropna(subset=["submit_date"])
                .drop_duplicates(subset=["code", "period_end"])
                .groupby("code").size().median()),
    },
    "specification_count": {
        "as_of_dates": len(AS_OF), "series": 3, "ranking_formulas": 2,
        "retained": len(AS_OF) * 3 * 2,
        "discarded_earnings_yield": len(AS_OF) * 3,
        "total": len(AS_OF) * 3 * 2 + len(AS_OF) * 3,
        "note": ("The total counts the discarded earnings-yield branch: we ran it before discovering "
                 "the share-count coverage problem, so it belongs in the multiplicity count even "
                 "though its results are not reported."),
    },
