...
def fig2_shu():
    # WP0.5/図の整合: 関門ごとの「通過後の残り社数」を正典funnel CSVから転記(v4の1段ずれを是正)
    _fn = {r["step"]: r for r in csv.DictReader(
        open(ROOT / "outputs/phase1_final/phase1_final_screening_funnel.csv", encoding="utf-8"))}
    _after = lambda s: int(_fn[s]["count_after"])
    stages = [
        ("全対象 ― 金融を除く普通株", int(_fn["A_universe_value_liquidity_available"]["count_before"])),
        ("割安か ― 純資産・利益で測る", _after("B_value")),
        ("収益力は本物か ― 粗利÷総資産", _after("C_quality")),
        ("財務は健全か ― 6項目チェック", _after("D_financial_strength")),
        ("利益は本物か ― 現金の裏づけ", _after("E_earnings_quality")),
        ("危険はないか ― 債務超過など除外", _after("F_distress")),
        ("実際に買えるか ― 売買のしやすさ", _after("G_liquidity")),
        ("完成した堀 Top5", 5),
    ]
    n_st = len(stages)
...
def fig2_ha():
    ...
    ax.text(1.7, 2.25, "多数の組合せを試し、効用が\n最大の設定を採用(採用後に\n安定性を監査)", ha="center", va="top", fontsize=9)
...
def fig3_cum():
    ...
    cum = _pd.read_csv(ASSETS / "cum3y_series_v5.csv", parse_dates=["date"]).set_index("date")
    cum = cum / cum.iloc[0]
...
if __name__ == "__main__":
    ...
    # v5で変わるのはfig2_shu(ファネル是正)・fig3_cum(TOPIX欠測補完)・fig2_ha(イテレーション13: 図内の
    # 禁止語「最も安定な設定」を裁定9どおり「効用が最大の設定を採用(採用後に安定性を監査)」へ是正)。
    # 他4図(fig1_moat/fig2_ri/fig2_ablation/fig3_roles)は目視監査済み・不変のためv4共有を使用。
    fig2_shu(); fig3_cum(); fig2_ha()
    print("done (v5: fig2_shu_v5, fig3_cum_v5, fig2_ha_v5)")
