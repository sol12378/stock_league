# -*- coding: utf-8 -*-
"""v7 査読用技術補遺(beyond_buffett_technical_appendix_v7.docx)。
...
# ===== 表紙 =====
p = doc.add_paragraph(); _font(p.add_run("BEYOND BUFFETT ― 査読用技術補遺(Technical Appendix) v7"), GOTHIC, 16, True, NAVY)
para("日経ＳＴＯＣＫリーグ提出版『三世代の堀』の検証可能性を担保する技術資料。本編(≤30頁)に収まらない詳細――"
     "全パラメータ・選定規律・計算規約・統計・頑健性・限界・再現手順――を収載する。全数値は本編と同一のv7正典"
     "(JSON)から自動転記。**重要な前提: 全ポートフォリオは2026-06時点の財務・価格で選定し過去に当てた in-sample の"
...
para(f"ユニバースは金融を除く普通株{FUNNEL['start_n']:,}社(価格あり)。うち投資適格かつ流動性(60日平均売買代金)を満たす"
     f"{FUNNEL['eligible_n']:,}社を選定の出発点とする。母集団は直近時点の上場一覧の単一断面で、期間中の上場廃止企業を"
     "含まない(生存者バイアス)。本ＰＦ・新バフェット・グレアムには同一条件で作用するため相互比較は内部整合するが、"
     "指数との水準比較は割り引いて読む。", after=6)
...
table(["守の品質関門", "通過社数(累積)"],
      [[f["label"], f"{f['n']:,}社"] for f in FUNNEL["funnel"]], [96, 40])
para(f"通過した品質ユニバース{FUNNEL['quality_universe_n']}社のうち、時価総額データで価格ランクできる{FUNNEL['priceable_n']}社を、"
     "収益力(ＲＯＥ)と益回りの順位和(Greenblatt型)で並べ、同一業種上限2で上位5社を完成した堀に固定した。", after=6)
h2("B-2　破 ― 変わる堀(割安×変革)")
para("破は、守の品質一辺倒を破り、東証の資本効率改革・脱炭素で評価がこれから変わる割安企業を選ぶ。study の変革カテゴリ"
     ...
     "設計し、±20%の摂動でも順位が崩れないことを監査した。", after=6)
...
para("離は、AI・半導体・光通信の実需で新しい堀が生まれる企業を選ぶ。ただし study の future_moat スコアは社名・業種への"
     ...
     "疑わしきは除外)。採用5社=santec(AIデータセンター向け光部品)/日本マイクロニクス(半導体プローブカード)/"
     "芝浦メカトロニクス(半導体製造装置)/SAMCO(化合物半導体装置)/テラプローブ(半導体ウエハテスト)。", after=3)
...
para("1. 提出PF・多期間: scripts/... build_portfolio_v7.py → work/pure_buffett_benchmark/portfolio_v7.json\n"
     ...
     "6. 図6点: build_figs_v7.py / build_fig_cum_v7.py → assets/*_v7.png\n"
     "7. 本編(2パス): build_contest_v7.py → soffice pdf → make_tocmap.py → build_contest_v7.py --tocmap tocmap_v7.json → soffice pdf\n"
     "8. 本補遺: build_appendix_v7.py → soffice pdf", mono=True, after=6)

out = ED / "beyond_buffett_technical_appendix_v7.docx"
if (ED / "beyond_buffett_technical_appendix_v7.LOCKED").exists():
    raise SystemExit("v7 appendix LOCKED")
doc.save(str(out))
