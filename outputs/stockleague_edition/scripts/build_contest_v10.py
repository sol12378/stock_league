...
body("第二の変化はＡＩを起点とする産業構造の転換である。生成ＡＩは、半導体・光通信・データセンター・電力・品質保証といった基盤産業に、実際の需要という追い風を与えている。ただし「ＡＩ」と口にする企業がすべて堀を持つわけではない。既製のＡＩ関連スコアは機械的なキーワード照合に頼るため、炊飯器や照明の会社までＡＩ関連として満点に並んでしまう。接点と堀を見分ける仕組み――事業の開示まで遡って実需を確かめる証拠主義――が要る。")
...
body("バフェットを超えるということを、確かめられる四つの条件で先に定義しておく。本レポートの残りは、この定義の検証である。")
...
table(["段階", "選ぶ堀とやること", "どこまでが借り物か", "検証する条件"],
      [["守", "完成した堀5社を、先行研究の式だけで選ぶ", "式も使い方も、すべて先行研究のまま", "条件①"],
       ["破", "型を破り、割安×変革の会社(変わる堀)5社を選ぶ", "式は借り物、使い方は私たちの判断", "(条件②の一部)"],
       ["離", "生まれる堀＋両立・分散の15社を、独自の物差しで選ぶ", "部品は借り物、組み方が独自", "条件②"]],
      [16, 72, 56, 26], fs=10)
para("図表Ⅰ-3　守・破・離の分担 ― 独自性を最後(離)に置く。", MIN, 10, after=5)
body("なお、本文の数字はすべて、選定の過程で保存した記録(通過社数・除外理由・検査結果)から転記ミスがないよう照合して記載した。計算に使ったデータと手順も保存しており、出どころまで遡って確かめられる。")
...
table(["「超える」の条件", "結果", "判定"],
      [["① 同じ土俵に立つ", "品質基準を式から再現し、その完成した堀5社を固定保有(第Ⅱ章)", "達成"],
       ["② 死角に届く", f"品質レンズが映さない13社を、事業で検証した証拠つきで選定(証拠水準3=約{_evid3}社・水準2=約{_evid2}社)。うち2社は彼の式にも入る＝堀が数字に現れ始めた会社", "達成"],
       ["③ 壊れにくい", f"目安{ROB['n_variants']}通り・配分3方式・相場4局面で検査。守5は最小{ROB['min_overlap']}/5一致(第Ⅱ章末)", "達成"],
       ["④ 総体で上回る", f"当時組めた新バフェット型(期首加重)に、3年は6つの物差しすべて・1年はβを除く5つで上回る(3年＋{D3:.1f}pt・1年＋{D1:.1f}pt。1年はｔ＝{SIG['1y']['ours_vs_buffett20_start']['t_newey_west']:.2f}で有意、3年は有意差なし)。グレアム型・市場には有意に上回る。ただし期末加重の仮想対照にはリターンで{-DE3:.1f}pt届かない(効率・下落では上回る)", "達成\n(3年は有意差なし)"]],
      [28, 112, 30], fs=10, fixed=True)
para("図表Ⅲ-6　判定表 ― 四条件に対する現時点の答え(すべて2026年時点選定の自己検証)。", MIN, 10, after=6)
body("第Ⅰ章の投資仮説――三世代を束ねたポートフォリオは、堀の“現在”だけを測る選定を、効率と下落の小ささで上回る――は、主対照に対して支持された。効率(シャープ)・下落・βのいずれも上回り、四つの相場局面すべてで下落は小さかった。ただし仮説を離れた事実も残る。上がった後の大きさで配分し直した仮想の対照には、リターンで届かない。胸を張れるのは成績の大きさではなく、比べる相手を自分に都合よく作らなかったこと、そして三世代の堀という枠組みを事前規律(後から入れ替えない・事業で証拠を確かめる)で実行したことにある。すべて2026年時点の選定を過去に当てた自己検証であり、将来の約束ではない。")

body(f"最後に、この設計の弱点を私たち自身の手で記録しておく――強みだけを並べたレポートは検証に耐えないからである。①テーマの偏り: 値上がり寄与の約{ROLE_CONTRIB['by_role_pct'].get('離 生まれる堀',0):.0f}%は生まれる堀(ＡＩ・半導体)に集中し、テーマの重みは約40%(業種の集中度ＨＨＩ＝0.16。0に近いほど分散、1に近いほど一つの業種に集中)。『ＡＩ・半導体バリューチェーンへの意図的な賭け』として隠さず開示する。テーマが崩れる未来では市場に劣りうるため、守・破に予算を置いて緩衝している。②単元未満株の前提: 株価の高い{N_UNBUY}社は通常の売買単位(100株)では¥500万に収まらず、本配分は単元未満株(金額指定)の取扱いを前提とした目標ウェイトである。③証拠は開示ベース: 現場の実態までは測れず、第Ⅳ章の取材はこの限界を埋めるために設計した。④出発点は生存者のみ: 出発点の{F9['n_nonfin']:,}社は期間中の上場廃止企業を含まず(生存者バイアス)、成績はその分有利に出うる。⑤対照の作り方に結論が左右される: 対照の配分をいまの会社の大きさに替えると、対照が本ＰＦのリターンを上回る(図表Ⅲ-4)。私たちは未来の情報を入れない作り方を主系列に選んだが、この選択自体が結論を動かすことは開示しておく。")
point("当時組めた新バフェット型(期首加重)には3年で6つの物差しすべて上回り、グレアム型と市場には有意に超えた。上がった後の大きさで配分した仮想対照にはリターンで届かない――そこまで含めて開示した。")
