# -*- coding: utf-8 -*-
"""v8 査読用技術補遺(beyond_buffett_technical_appendix_v8.docx)。
...
# ===== 表紙 =====
p = doc.add_paragraph(); _font(p.add_run("BEYOND BUFFETT ― 査読用技術補遺(Technical Appendix) v8"), GOTHIC, 16, True, NAVY)
...
para("1. 提出PF・多期間: scripts/... build_portfolio_v7.py → work/pure_buffett_benchmark/portfolio_v7.json\n"
     ...
     "7. 本編(2パス): build_contest_v8.py → soffice pdf → make_tocmap.py → build_contest_v8.py --tocmap tocmap_v8.json → soffice pdf\n"
     "8. 本補遺: build_appendix_v8.py → soffice pdf", mono=True, after=6)

out = ED / "beyond_buffett_technical_appendix_v8.docx"
if (ED / "beyond_buffett_technical_appendix_v8.LOCKED").exists():
    raise SystemExit("v8 appendix LOCKED")
doc.save(str(out))
