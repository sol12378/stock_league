...
out = {
    ...
    # A referee will ask whether the placebo pool actually contains the portfolio. If it did not,
    # the draws would not be a superset of the thing being evaluated and the percentile would be
    # meaningless. Verified rather than assumed.
    "holdings_inside_pool": int(sum(1 for c in codes_actual if c in set(pool_codes))),
    "holdings_total": len(codes_actual),
