...
FUNNEL = json.load(open(ED / "funnel_exclusion_v7.json", encoding="utf-8"))
...
body(f"前章では「超える」を四つの条件で定義し、選ぶ順番として守破離を置いた。本章が選定の中核である。金融を除く普通株{FUNNEL['start_n']:,}社(投資適格・流動性を満たす{FUNNEL['eligible_n']:,}社)から、守破離の三段階で最終20社を選ぶ。各段階では、何を見たか(観点)、何で測ったか(データと式)、どこで区切ったか(判定の目安)、何社が残ったか(結果)を必ず示す。データの基準は、価格が2023年4月25日〜2026年6月1日の日次、財務・開示がＥＤＩＮＥＴ提出書類(2026年7月8日取得。各社は取得日時点の直近有価証券報告書で、決算期の違いにより最大12カ月の鮮度差がある)である。")
...
verdict("よって、ＲＯＥ15％以上の企業を「資本を高い率で回せる優良企業」と評価する。")
...
verdict("よって、営業利益率10％以上の企業を「価格支配力＝堀を持つ」と評価する。")
...
verdict("よって、自己資本比率50％以上の企業を「財務が健全」と評価する。")
...
verdict("よって、直近3期に一度も赤字のない企業を「利益が予測可能」と評価する。")
...
verdict("よって、本業で現金を生んでいる企業を「利益が本物」と評価する。")
...
verdict("よって、増収かつ増益の企業を「事業が縮小していない」と評価する。")
...
table(["品質関門", "レーザーテックの値", "判定の目安", "判定"],
      ...
      [42, 44, 40, 20], fs=10)
para("図表Ⅱ-2　通し数値例：レーザーテック(6920)の品質関門。出所：選定の正典データ(data_real_v7)より筆者作成。", MIN, 10, after=6)
b5 = [DATA[c] for c in ["3092", "4716", "7014", "8136", "6920"]]
body(f"守の品質関門を通過した{FUNNEL['quality_universe_n']}社は、破の出発点であり、後の比較の基準線でもある。この中から、価格ランクが可能な{FUNNEL['priceable_n']}社を、割安×優良の複合順位(収益力ＲＯＥと益回りの順位和＝Greenblattの考え方)で並べ、同一業種は原則2社までとして上位5社を固定した。選ばれたのは、"
     + "、".join(f"{d['code']} {d['name_ja']}({SEC_JP.get(d['sector'], d['sector'])})" for d in b5)
     + "の5社である。以降、この5社は一度も入れ替えない。")
fig("fig2_shu_v7.png", f"図表Ⅱ-3　守の絞り込み ― 適格{FUNNEL['eligible_n']:,}社から完成した堀Top5へ",
    "注：棒の幅と色の濃さが残った会社の数を表す(新バフェット品質ゲートの累積)。出所：funnel_exclusion_v7.json。")

step_chip(2, "破 ― 型を破り、割安×変革の会社(変わる堀)を選ぶ")
body("前節の守では、新バフェットの品質基準で完成した堀の5社(基準線)を決めた。本節の破では、その品質一辺倒の型を一度『破り』、いまは割安に放置されているが、東証の資本効率改革や脱炭素の流れでこれから評価が変わる会社(変わる堀)を探す。守の高い品質関門だけでは、割安×変革の候補が抜け落ちてしまうからだ。ここで候補を評価するために、指標を公平に比べる順位化(式8)と、変革の度合いを束ねる合成点(式9)を用意する。")
...
body("重みについて、一つ正直に書き残す。重みは過去リターンへの当てはめでは決めていない――どの重みなら過去の成績が良かったか、という決め方は後出しの罠だからである。『安さ』より『改善の実行』を重くする、という変革の論理から概念的に設計し、決めた後に各重みを±20％動かしても選ばれる顔ぶれが変わらないことを監査した(全重みと監査結果は技術補遺§C)。重みの大きさを予測力と読んではいけない。")
banner3("手順3　順位の付け方を変えても結果は同じか ― 頑健さの監査")
body("順位化のやり方は一つではない。市場全体での順位・業種の中での順位・外れ値に強い標準化など複数のやり方で変わる堀の順位を付け直し、選ばれる顔ぶれが入れ替わらないかを確かめた。付け方に敏感な会社には確認フラグを付け、離の個別確認へ引き継いだ(詳細は技術補遺§C・§E)。", after=2)
table(["破の観点", "内容", "結果"],
      [["順位化(式8)", "各シグナルを市場の中の順位(0〜1)に置き換える。式の定義は変えない", "単位の違う指標を公平に比較"],
       ["合成点(式9)", "変革の論理で重みを設計(過去成績では決めない)。採用後に±20％動かす検査", "順位は動かしても頑健"],
       ["頑健さの監査", "順位の付け方を複数変えて顔ぶれの安定を確認", "敏感な社は確認フラグ→離へ"],
       ["選定", "Transformation Moat(変革の堀)分類・黒字・流動性・同一業種2社まで", "変わる堀5社"]],
      [28, 88, 54], fs=10)
para("図表Ⅱ-4　破のやり方 ― 式は変えず、変革の度合いで割安×変革の会社を選ぶ。出所：選定の正典データ。", MIN, 10, after=5)
...
fig("fig2_ha_v7.png", "図表Ⅱ-5　破 ― 変わる堀の合成点で割安×変革の会社を選ぶ",
    "注：破の点数は候補選びの道具であり、最終選定の重みには使わない。筆者作成。")

step_chip(3, "離 ― 生まれる堀を測り、証拠で束ねて最終20社を組む")
body("破と離では、私たち独自の物差し(式10〜式12)を導入する――変わる堀の点数(式10・破が使う)、生まれる堀の点数(式11・離が使う)、そして両者に共通の証拠の関所(式12)である。ただし新しい式をゼロから発明したのではなく、その導出は三つの原則に従う。第一に、部品は先行研究と守の式の再利用――測る量そのものは、割安(式1・式2)・利益の質(式5)・無形資産の文献に既にあるものだけを使う。第二に、重みは成績で決めない――どの重みなら過去の成績が良かったか、という決め方は後出しの罠なので、重みは投資テーマの論理から概念的に設計し、決めた後に「重みを揺らしても順位が壊れないか」を監査する。第三に、点数と証拠の分離――点数(魅力の大きさ)は連続値で設計の裁量が入るが、証拠(裏づけの強さ)は開示資料まで遡って誰でも確認できる。この二つを混ぜず、証拠を式(12)の独立した関所にする。以下、変わる堀(式10)・生まれる堀(式11)・証拠(式12)の順に「なぜこの形か」を説明する。")
banner3("式(10)の導出 ― 破の「変わる堀の点数」を要素に分ける")
body("式(9)の合成点を、変わる堀に合わせて具体化したものが式(10)である(破の選定に使う独自の物差し)。変わる堀の定義『割安に放置されているが、資本効率改革の実行でこれから評価が変わる会社』を、四つの測れる要素に分ける――(a)割安ギャップＶ(いま割安か=式1・2の順位点)、(b)資本効率の改善Ｃ(ＲＯＥ・利益率が良くなっているか)、(c)株主還元の強化Ｒ(配当・自社株買いの実行)、(d)改革シグナルＫ(中期計画・ＰＢＲ改善策の開示)。これらを重みつきで束ね、『安いだけの会社』を弾く減点を差し引く(study の変革スコア=transformation score)。", after=2)
formula("10", "変わる堀の点数(破の選定に使用・私たちの設計)",
        r"S^{\mathrm{Trans}}_i = w_V V_i + w_C C_i + w_R R_i + w_K K_i - P^{\mathrm{Trap}}_i",
        "Ｖ＝割安ギャップ(式1・2の順位点)　Ｃ＝資本効率の改善　Ｒ＝株主還元の強化　Ｋ＝改革シグナル(開示)　Ｐ＝割安のワナ減点。改善の実行(Ｃ・Ｒ・Ｋ)を割安(Ｖ)より重くする。")
body("重みは過去リターンへの当てはめでは決めない(後出しの罠を避ける)。『安さ』より『改善の実行』を重くする、という変革の論理から概念的に設計し、決めた後に各重みを±20％動かしても選ばれる顔ぶれが変わらないことを監査した(全重みと監査結果は技術補遺§C)。減点は「安いだけの会社」の既知の型――利益に現金の裏づけがない・財務が危険・赤字が続く等――を列挙して点数から差し引く。", after=2)
verdict("よって、割安さに「変革の実行」が重なる会社ほど高得点とし、変わる堀の上位5社を選ぶ。安いだけの会社は減点で弾く。")
...
body("重みは、中心仮説「ＡＩ基盤への実需接続が新しい堀を生む」からＡ(ＡＩ接続)を最重視し、堀の古典的源泉(無形資産・急所を握る度合い)と技術力が続く――過去リターンでは決めない。加点は証拠水準が1段上がるごとに2点→4点→8点と倍にした(水準3=数字まで確認はそれだけ希少)。ただし study の future_moat スコアはキーワードで飽和し、炊飯器や照明の会社まで満点に並ぶことが分かったため、私たちは点数を鵜呑みにせず、事業セグメントの開示まで遡ってＡＩ・半導体の実需を確認できた会社だけを通した(キーワード頼みの排除)。全重みは技術補遺§Bに開示した。", after=2)
verdict("よって、ＡＩ基盤への実需接続を事業の開示で確認できる会社ほど高得点とし、生まれる堀の上位5社を選ぶ。キーワードの接点しかない会社は除外した。")
...
body("当てはめ例: 両立型のキーエンス(6861)は「変わる証拠(高収益の持続)」と「新テーマの証拠(工場自動化・ＡＩの実需)」の弱い方(min)で判定する――どちらの裏づけも事業の開示で確認できて初めて、両立型として通す。", after=4)
body(f"守の完成した堀は、バフェットの品質基準を一つずつ課して選ぶ。適格{FUNNEL['eligible_n']:,}社から、各関門で落ちた会社は「どの基準を満たさなかったか」で理由が特定できる（下表）。落ちた理由をすべて残すことが、選んだ理由の裏づけになる。生まれる堀・両立型では、ＡＩ・半導体の接点をキーワードで数えず、事業セグメントの開示まで遡って実需を確認できた会社だけを通した（疑わしきは除外の保守設計）。")
_fn = {f["label"]: f["n"] for f in FUNNEL["funnel"]}
_seq = [f for f in FUNNEL["funnel"] if f["step"] >= 3 and f["step"] <= 8]
table(["守の品質関門（バフェット基準）", "通過社数", "落ちる理由"],
      [["高収益 ＲＯＥ≥15%", f"{_seq[0]['n']:,}社", "資本を高い率で回せていない"],
       ["堀・価格支配力 営業利益率≥10%", f"{_seq[1]['n']:,}社", "価格支配力＝堀の薄さ"],
       ["財務健全 自己資本比率≥50%", f"{_seq[2]['n']:,}社", "負債が重く長期保有に不向き"],
       ["予測可能性 直近3期無赤字", f"{_seq[3]['n']:,}社", "利益が不安定で読みにくい"],
       ["現金創出 営業ＣＦ>0", f"{_seq[4]['n']:,}社", "利益に現金の裏づけがない"],
       ["非縮小 増収かつ増益", f"{_seq[5]['n']:,}社", "事業が縮小している"]],
      [34, 16, 110], fs=10)
para(f"図表Ⅱ-6　守の品質ファネル（新バフェット基準の累積絞り込み）。適格{FUNNEL['eligible_n']:,}社→品質通過{FUNNEL['quality_universe_n']}社→価格ランク可能{FUNNEL['priceable_n']}社→完成した堀Top5。出所：funnel_exclusion_v7.json（scores.csvから自動集計）。", MIN, 10, after=6)
...
fig("fig2_ri_v7.png", "図表Ⅱ-7　離 ― 三世代の堀から最終20社へ(矢印は1対1の対応)",
...
bullet("第二段階", "役割ごとに予算を決める。完成・変化・新生の三世代に各28％、両立型10％、分散役6％。三世代へ同額を置くのは、どの未来にも賭けないという設計の続きである。")
bullet("第三段階", "同じ役割の中では、値動きが穏やかで・売買しやすく・証拠が強く・データが信頼できる会社を厚くする(式13)。")
bullet("第四段階", "1銘柄の比率に上限をかけ、1株から買える単元未満株(金額指定)で目標比率どおりに配分する(式14)。")
...
body(f"最終配分と20社の顔ぶれを、図表Ⅲ-1に一覧で示す（配分・事業概要・堀の偏差値を一表に集約）。役割の予算は守28／破28／離28／両立10／分散6％、最大の保有は{wmax['name_ja']}の{wmax['w']*100:.2f}％。新バフェット型は株価の高い大型優良が中心で通常の売買単位(100株)では予算内に収まらないため、1株から買える単元未満株(金額指定)の利用を前提に、目標比率で総額500万円を配分した。堀の偏差値は全上場企業の中での位置を平均50で表した値で、現在の堀＝いまの競争優位、未来の堀＝これから生まれる競争優位の強さを示す。")
prows = []
for role in order:
    for d in sorted([x for x in DATA.values() if x["role"] == role], key=lambda z: -z["w"]):
        prows.append([f"{d['code']} {d['name_ja']}", role_jp[role],
                      f"{d['w']*100:.1f}％\n{d['amtL1']:,}円", d.get("business", ""),
                      f"{d['moat_hensa']}", f"{d['fmoat_hensa']}"])
table(["企業（コード・社名）", "役割", "比率／金額", "事業概要（要ＩＲ確認）", "現在\nの堀", "未来\nの堀"], prows,
      [32, 18, 22, 78, 16, 16], fs=10)
para("図表Ⅲ-1　最終ポートフォリオ20社（総予算500万円・単元未満株の目標配分）。事業概要は各社ＩＲ資料で確認して記載。堀の偏差値は全上場企業の中での位置(平均50)。出所：portfolio_v7.json・data_real_v7.json。", MIN, 10, after=7)
...
for role in order:
    ...
    names = "／".join(f"{d['code']} {d['name_ja']}"
                     for d in sorted([x for x in DATA.values() if x["role"] == role], key=lambda z: -z["w"]))
    body(f"{names}。（配分・事業概要・堀の偏差値は図表Ⅲ-1に集約。各社の証拠水準と役割別スコアの詳細は技術補遺§Bに、事業内容と堀の背景は各社ＩＲ資料で確認のうえ記載する＝創作はしない。）", after=4)
...
body("位置づけに先に限定をつける。新バフェット型もグレアム型も『式だけの機械選定』であり、バフェット本人の運用(少数集中・保険フロートによる調達・数十年保有)そのものではない。したがってこの比較は「バフェット本人に勝った」ことを意味せず、『同じ土俵で式だけを使った場合に対し、三世代の設計が何を足したか』を測る基準線として読む。そしてすべては2026年時点の選定を過去に当てた自己検証であり、将来の勝利の証明ではない。")
fig("cum_v7.png", "図表Ⅲ-7　累積リターンの比較(過去3年・期首=1)",
    "注：本ＰＦ・新バフェット型・純正グレアム型・ＴＯＰＩＸ・日経平均。ＴＯＰＩＸ連動指数1306は未調整の株式分割(2026-03-30)を×10補正、指数系列の欠測を前営業日で補完。価格はyfinance調整後終値(配当込み。日経のみ価格指数)。すべて2026年時点選定の自己検証。出所：control_comparison_v7.json・価格正典データ。")
table(["物差し", "本ＰＦ(離)", "新バフェット型(主対照)", "ＴＯＰＩＸ", "日経平均"],
...
body(f"読み取りは三つ。第一に、本ポートフォリオは主対照の新バフェット型を、選定に使った3年間で年{D3:.1f}ポイント、直近1年でも年{D1:.1f}ポイント上回り、割安一辺倒のグレアム型には3年で年{DG3:.1f}ポイント上回った。ただし正直に言えば、新バフェット型への超過は統計的には確実でない――日次超過のNewey-Westｔ値は3年{SIG['3y']['ours_vs_buffett']['t_newey_west']:.2f}で有意水準に届かない。点推定と四つの相場局面すべてでの一貫性では上回るが、新バフェット型に対しては「互角〜やや上」と評価する。統計的に有意に上回るのは、グレアム型(ｔ＝{SIG['3y']['ours_vs_graham']['t_newey_west']:.2f})と市場ＴＯＰＩＸ(ｔ＝{SIG['3y']['ours_vs_topix']['t_newey_west']:.2f})に対してである。第二に、新バフェット型に対しては守り――最大の下落({O3['max_drawdown']:.1%} 対 {C3['max_drawdown']:.1%})と市場との連動度β({O3['beta_vs_topix']:.2f} 対 {C3['beta_vs_topix']:.2f})――でも上回った。過度な集中を避けた三世代分散が、攻めと守りの両面で効いている。第三に、これはすべて2026年時点の選定を過去に当てた自己検証であり、将来の市場超過の証明ではない――この限界は言い換えずに残す。")
body(f"役割の分担も確かめた(図表Ⅲ-9)。過去3年の値上がり寄与は、生まれる堀(ＡＩ・半導体)が約{ROLE_CONTRIB['by_role_pct'].get('離 生まれる堀',0):.0f}％と牽引し、完成した堀(守)も約{ROLE_CONTRIB['by_role_pct'].get('守 完成した堀',0):.0f}％と土台以上に寄与、変わる堀(公益・資源)は約{ROLE_CONTRIB['by_role_pct'].get('破 変わる堀',0):.0f}％で下げ相場の緩衝材として効いた。旧来の割安一辺倒(グレアム型)に比べ、三世代へ寄与が分散している点が特徴である。なお本検証は取引コスト・税を考慮せず、毎日同じ比率へ戻す固定重み規約で計算した。株数を固定して買い持ちにすると値動きが変わる(参考値は技術補遺§D)。")
fig("fig3_roles_v7.png", "図表Ⅲ-9　役割別の寄与 ― 生まれる堀が牽引し、守も土台以上に寄与",
...
table(["「超える」の条件", "結果", "判定"],
      [["① 同じ土俵に立つ", "守で新バフェットの品質基準(高収益・堀・財務健全)を式から再現し、その完成した堀5社を固定保有(第Ⅱ章)", "達成"],
       ["② 師に見えない領域へ届く", f"新バフェットの『いまの品質』レンズが映さない変わる堀・生まれる堀の15社を、事業で検証した証拠つきで選定(証拠水準3=約{_evid3}社・水準2=約{_evid2}社)", "達成"],
       ["③ 壊れにくい", "判定の目安・重み方式・相場局面の三通りの頑健性検査で確認。守5は動かしても最小4/5一致、『超え』は重み方式・局面に不変(第Ⅱ章末)", "達成"],
       ["④ 総体で上回る", f"新バフェット型を3年で年＋{D3:.1f}pt・1年で年＋{D1:.1f}pt上回り、グレアム型には3年＋{DG3:.1f}pt。有意に勝つのはグレアム型と市場ＴＯＰＩＸ、新バフェットへの超過は有意差なし＝互角〜やや上(図表Ⅲ-8)", "達成(新バフェットは有意差なし)"]],
      [30, 118, 22], fs=10)
para("図表Ⅲ-10　判定表 ― 四条件に対する現時点の答え(すべて2026年時点選定の自己検証)。", MIN, 10, after=6)
body(f"結局、超えたのか。答えは明快に分けて書く。割安一辺倒のグレアム型と市場平均(ＴＯＰＩＸ)には、リスクを抑えつつ統計的にも有意に上回った。師である新バフェット型に対しては、リターン・効率(シャープ)・最大の下落のすべてで、しかも利上げ相場からＡＩ相場まで四つの局面すべてで一貫して上回ったが、その差は統計的には確実とは言えない――正直に「互角〜やや上」と評価する。第Ⅰ章の投資仮説――三世代を束ねた選定はリスク調整後で師の型を上回る――は、この範囲で支持された。")
body("そして最も大切な限界を先に置く。以上はすべて2026年時点の選定を過去に当てた自己検証であり、将来の市場超過を約束するものではない。私たちが胸を張れるのは成績の大きさではなく、三世代の堀という枠組みと、それを事前規律(後から銘柄を入れ替えない・事業で証拠を確かめる)で実行したことにある。")

banner2("5．リスクと限界 ― 正直に書き残す")
body("最後に、この設計の弱点を私たち自身の手で記録しておく――強みだけを並べたレポートは検証に耐えないからである。")
bullet("テーマの偏り(意図的な賭け)", f"過去3年の値上がり寄与の約{ROLE_CONTRIB['by_role_pct'].get('離 生まれる堀',0):.0f}%は生まれる堀(ＡＩ・半導体)に集中し、ＡＩ/半導体テーマの重みは約40%(業種HHI0.16)と高い。これは『生まれる堀＝ＡＩ・半導体バリューチェーンへの意図的な賭け』であり隠さず開示する。テーマが崩れる未来では市場に劣りうるため、守(大型優良)・破(変革・公益)に予算を置いて緩衝している。")
bullet("単元未満株の前提", f"新バフェット型は大型優良で株価が高く、通常の売買単位(100株)では株価の高い{N_UNBUY}社が¥500万の予算内で買えない。本配分は1株から買える単元未満株(金額指定)の取扱いを前提とした目標ウェイトであり、取扱いのない証券会社ではそのまま再現できない。")
bullet("証拠は開示ベース", "証拠の強さは開示資料に基づく判定であり、現場の実態までは測れない。第Ⅳ章の取材は、この限界を埋めるために設計している。")
bullet("出発点は生存者のみ", f"出発点の{FUNNEL['start_n']:,}社は直近時点の上場一覧で、期間中に上場廃止となった企業を含まない(生存者バイアス)。成績はその分有利に出うる(技術補遺§A)。")
point("グレアム型と市場には有意に超え、新バフェットには全指標・全局面で上回るが有意差なし(互角〜やや上)。すべて自己検証であること、テーマ集中とデータの限界まで、先に書き残した。")
