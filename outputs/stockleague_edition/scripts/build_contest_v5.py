...
table(["関連の深いＳＤＧｓの目標", "その主な理由(記入)"],
      [["9．産業と技術革新の基盤をつくろう", "［記入：生まれる堀=ＡＩ基盤への投資と技術革新の関係］"],
       ["8．働きがいも経済成長も", "［記入：変わる堀=資本効率改革と経済成長の関係］"],
       ["12．つくる責任・つかう責任", "［記入］"]],
...
body("7つの順位点に重みを付けて足し合わせ、危険な兆候(異常値・データ欠け・極端に小さい会社・一時的な利益)には減点を与える。重み・減点の強さ・候補数・業種調整の有無・データ欠けの扱いは、無数の組合せを機械的に大量に試す方法(ランダムサーチ他)で比較し、候補集合の効用(広さ・質・安全・業種分散など)が最も高くなる設定を採用した(採用後に順位の安定性を別途監査した。目的関数と探索の詳細は技術補遺§C・§E)。採用した設定は「市場順位で0〜1化・主要指標が欠けた会社は除外・業種調整はしない」である。", after=2)
...
formula("10", "変わる堀の点数(選定に使用)",
        ...
        "Ｖ＝割安さ　Ｃ＝資本効率の改善　Ｆ＝還元の余力(本業の現金収入−設備投資)　Ｘ＝実行の裏づけ(改善が財務の質に表れているか)　Ｑ＝利益の質　Φ＝データの信頼度　Ｐ＝割安のワナ減点。")
...
fig("fig2_ablation.png", "図表Ⅱ-8　条件を一つ変えて選び直したとき、最終20社のうち何社が残るか",
    "注：過去データ上の構造の検査であり、成績の主張ではない。出所：検査結果の正典データ(ablation_results.csv)。", maxw=122)
ABL_JP = {
...
fam_rows = [[f, "・".join(m), f"{min(ABL_OV[x] for x in m)}〜{max(ABL_OV[x] for x in m)}/20", y]
            for f, m, y in FAMS]
table(["検査の家族", "検査", "一致の範囲", "読み取り"], fam_rows, [38, 42, 26, 64], fs=10, keep_caption=True)
para("図表Ⅱ-9　16通りの検査の4家族要約(各検査の個別値は図表Ⅱ-8に記載)。一致＝選び直しても最終20社と同じだった社数。――本章の要点: 守で基準線5社(条件①)、離で死角を補う15社(条件②)を選び、16通りの検査で壊れにくさ(条件③)を確かめた。", MIN, 10, after=6)
